...
void IUI::IUILockGlobals(int nLockType)
{
...
#ifdef _DEBUG
	_ASSERT(++_afxResourceLocked[nLockType] > 0);
#endif
}

void IUI::IUIUnlockGlobals(int nLockType)
{
	_ASSERT(_afxCriticalInit);
	ENSURE((UINT)nLockType < CRIT_MAX);

	// unlock specific resource
	_ASSERT(_afxLockInit[nLockType]);
#ifdef _DEBUG
	_ASSERT(--_afxResourceLocked[nLockType] >= 0);
#endif
