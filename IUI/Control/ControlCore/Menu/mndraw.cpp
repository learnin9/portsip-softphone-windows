...
{
	return FALSE;//TestMFS(pItem, MFS_CACHEDBMP) && (pItem->lpstr == NULL);
}
/***************************************************************************\
* MNDrawHilite
*
...
{
	HBRUSH hbrTopLeft = NULL;
	HBRUSH hbrBottomRight = NULL;
	BOOL fDrawEdge;
	...
	if (!TestMF(pMenu, MFISPOPUP))
	{
		...
		{
			hbrTopLeft = SYSHBR(threeDSHADOW);
			hbrBottomRight = SYSHBR(threeDHILIGHT);
			SetMFS(pItem, MFS_HOTTRACKDRAWN);
			fDrawEdge = TRUE;
		}
		else if (TestMFS(pItem, MFS_HOTTRACK))
		{
			hbrTopLeft = SYSHBR(threeDHILIGHT);
			hbrBottomRight = SYSHBR(threeDSHADOW);
			SetMFS(pItem, MFS_HOTTRACKDRAWN);
			fDrawEdge = TRUE;
		}
		else if (TestMFS(pItem, MFS_HOTTRACKDRAWN))
		{
			hbrTopLeft = SYSHBR(MENUHBR);
			hbrBottomRight = SYSHBR(MENUHBR);
	...
	if (fDrawEdge)
	{
		...
		HBRUSH hbrOld = (HBRUSH)SelectObject(hdc, hbrTopLeft);
		PatBlt(hdc, x, y, cx - CXMENU3DEDGE, CYMENU3DEDGE, PATCOPY);
		PatBlt(hdc, x, y, CXMENU3DEDGE, cy - CYMENU3DEDGE, PATCOPY);
		SelectObject(hdc, hbrBottomRight);
		PatBlt(hdc, x, y + cy - CYMENU3DEDGE, cx - CXMENU3DEDGE, CYMENU3DEDGE, PATCOPY);
		PatBlt(hdc, x + cx - CYMENU3DEDGE, y, CXMENU3DEDGE, cy, PATCOPY);
...
{
	HWND    pwnd = ppopup->spwndPopupMenu;
	HDC     hdc;
	int     x, y;
	DWORD   dwBmp;
	DWORD   dwAtCheck;
	...
	if (hdcIn == NULL)
	{
		hdc = GetDCEx(pwnd, NULL, DCX_USESTYLE | DCX_WINDOW | DCX_LOCKWINDOWUPDATE);
	}
	else
	{
		hdc = hdcIn;
	...
	{
		y = rcWindow.bottom - rcWindow.top - SYSMET(CYFIXEDFRAME) - gcyMenuScrollArrow;
	...
	if (ppopup->spmenu->hbrBack != NULL)
	{
		...
		rc.top = y;
		rc.left = x;
		rc.right  = x + 10;//gpsi->oembmi[OBI_MENUARROWUP].cx;
		rc.bottom = y + 10;//gpsi->oembmi[OBI_MENUARROWUP].cy;
...
\***************************************************************************/
void MNDrawFullNC(HWND pwnd, HDC hdcIn, PPOPUPMENU ppopup)
{
	RECT rc;
	HDC hdc;
	HBRUSH hbrOld;
	...
	if (ppopup->spmenu->hbrBack != NULL)
	{
		...
		hbrOld = (HBRUSH)SelectObject(hdc, ppopup->spmenu->hbrBack);
	}
	else
	{
		hbrOld = (HBRUSH)SelectObject(hdc, SYSHBR(MENUHBR));
	}

	rc.right -= rc.left;
	PatBlt(hdc, rc.left, yTop, rc.right, gcyMenuScrollArrow, PATCOPY);
	MNDrawArrow(hdc, ppopup, MFMWFP_UPARROW);
	PatBlt(hdc, rc.left, yBottom, rc.right, gcyMenuScrollArrow, PATCOPY);
...
void MNEraseBackground(HDC hdc, PMENU pmenu, int x, int y, int cx, int cy)
{
	...
	if (fSetOrg)
	{
		SetBrushOrgEx(hdc, ptOrg.x, ptOrg.y, &ptOrg);
	}
	hbrOld = (HBRUSH)SelectObject(hdc, pmenu->hbrBack);
	...
	(HBRUSH)SelectObject(hdc, hbrOld);
}
...
void MNAnimate(PMENUSTATE pMenuState, BOOL fIterate)
{
	 ...
	 */
	_ASSERT(IsWindowVisible(pMenuState->pGlobalPopupMenu->spwndActivePopup));

	/*
	 * End animation if asked to do so, if it's taking too long
	 *  or someone is waiting for the critical section
	 */
	dwTimeElapsed = GetTickCount() - pMenuState->dwAniStartTime;
	if (!fIterate
		...
		|| (ExGetSharedWaiterCount(gpresUser) > 0)*/)
	{
		...
		BitBlt(pMenuState->hdcWndAni, 0, 0, pMenuState->cxAni, pMenuState->cyAni, pMenuState->hdcAni,
	...
	{
		pMenuState->ixAni = MulDiv(gcxMenuFontChar, dwTimeElapsed, CMS_QANIMATION / 20);
	...
	{
		pMenuState->iyAni = MulDiv(gcyMenuFontChar, dwTimeElapsed, CMS_QANIMATION / 10);
	 ...
	 * Do it
	 */
	BitBlt(pMenuState->hdcWndAni, x, y, pMenuState->ixAni, pMenuState->iyAni,
		pMenuState->hdcAni, xOff, yOff, SRCCOPY | NOMIRRORBITMAP);
...
AnimationCompleted:
		...
		ReleaseDC(WindowFromDC(pMenuState->hdcWndAni), pMenuState->hdcWndAni);
		pMenuState->hdcWndAni = NULL;
		KillTimer(pMenuState->pGlobalPopupMenu->spwndActivePopup, IDSYS_MNANIMATE);
	}
...
BOOL DrawMenuItemCheckMark(HDC hdc, PITEM pItem, int xPos)
{
	...
	if (fChecked)
	{
		CIUIRect rcCheck(xPos, yCenter, xPos + 20, yCenter + 24);
		HBRUSH h = ::CreateSolidBrush(RGB(0, 0, 255));
		FillRect(hdc, rcCheck, h);
		::DeleteObject(h);
	}
	...
	LPWSTR pszMenu, LONG lResLo)
{
	...
	PatBlt(hdc, xLeft, yTop, pItem->ulWidth, SYSMET(CYBORDER), PATCOPY);
}
	...
	LPWSTR  lpsz, int cch, BOOL fShowUnderlines)
{
	...
	if (cch > MENU_STRLEN)
	{
		pchOut = new WCHAR[cch + 1];//(WCHAR *)UserAllocPool((cch + 1) * sizeof(WCHAR), TAG_RTL);
		if (pchOut == NULL)
		{
			return;
		}
	...
	// scott add
	DrawTextW(hdc, lpsz, cch, CIUIRect(xLeft, yTop, pItem->cxItem, yTop + pItem->cyItem), DT_SINGLELINE);
	...
	//else
	//{
	//	ExtTextOutW(hdc, xLeft, yTop, 0, NULL, pchOut, cch - HIWORD(result), NULL);
	//}
	...
	int iOffset)
{
	...
	if (pmenu->spwndNotify != NULL)
	{
		...
		SendMessage(pmenu->spwndNotify, WM_DRAWITEM, 0, (LPARAM)&dis);
	...
	DWORD dwFlags)
{
	...
	if (TestMFS(pItem, MFS_DEFAULT))
	{
		...
		{
			hfnOld = (HFONT)SelectObject(hdc, ghMenuFontDef);
		}
		else
		{
			uFlags |= DSS_DEFAULT;
			tcExtra = GetTextCharacterExtra(hdc);
			SetTextCharacterExtra(hdc, tcExtra + 1 + (gcxMenuFontChar / cxSysFontChar));
		}
	}
	...
	if (TestMFT(pItem, MFT_OWNERDRAW))
	{
		 ...
		 */
		::SetTextColor(hdc, SYSRGB(MENUTEXT));
		::SetBkColor(hdc, SYSRGB(MENUHBR));
	...
	else
	{
			...
			&& (TestMFS(pItem, MF_GRAYED) || TestMF(pMenu, MFINACTIVE)))
		{
			...
			if ((icolBack == COLOR_HIGHLIGHT) ||
				(SYSRGB(MENUHBR) != SYSRGB(treeDFACE)) || SYSMET(SLOWMACHINE))
			{
				//
			...
			{
				if ((SYSRGB(threeDSHADOW) == SYSRGB(MENUHBR)) && (SYSRGB(threeDHILIGHT) == SYSRGB(MENUHBR)))
				{
					uFlags |= DSS_UNION;
				}
				else
		...
		::SetTextColor(hdc, SYSRGB(treeDFACE));
		...
		{
			iBkSave = SetBkMode(hdc, TRANSPARENT);
		}
		(HBRUSH)SelectObject(hdc, SYSHBR(MENUHBR));
		...
		BOOL fResult = xxxRealDrawMenuItem(hdc, &gm, pItem->cxItem, pItem->cyItem);
	...
	if (TestMFS(pItem, MFS_DEFAULT))
	{
		if (hfnOld)
		{
			(HFONT)SelectObject(hdc, hfnOld);
		}
		else
		{
			SetTextCharacterExtra(hdc, tcExtra);
		}
	}
}
...
*
\***************************************************************************/
BOOL CALLBACK IUI::xxxRealDrawMenuItem(
	HDC hdc,
	PGRAYMENU pGray,
	int cx,
	int cy)
{
	...
	//				cyUse, 0, SYSHBR(MENUHBR), DI_NORMAL);
	//
	//		}
	//		else
	//		{
	...
	//		BitBlt(hdc, xLeft + xHilite, cyTemp + xHilite, dx, dy, ghdcMem2, 0, 0,
...
RealDrawMenuItemText:
	if (pItem->lpstr != NULL)
	{
		...
		// 原版是：yTop = gcyMenuFontExternLeading;
		yTop = pItem->yItem;
		...
		if (lpsz != NULL)
		{
			...
			if (rp && (rp != cch))
			{
				...
				xxxDrawMenuItemText(pItem, hdc, xLeft + xHilite, yTop, lpsz, rp,
			...
			else if (tp && rp == cch)
			{
				...
				xxxDrawMenuItemText(pItem, hdc, xLeft + xHilite, yTop, lpsz, tp,
					TestMF(pMenu, MFUNDERLINE));
			}
	...
	{
		BOOL bLeftArrow = TestMFT(pItem, MFT_RIGHTORDER);

		CIUIRect rc(pItem->cxItem - 20, pItem->yItem, pItem->cxItem, pItem->yItem + pItem->cyItem);
		DrawText(hdc, _T(">"), 1, rc, DT_SINGLELINE | DT_VCENTER | DT_CENTER);
	}
	...
	int cyFrame)
{
	...
	//		PolyData[0].BrClr.hbr = (pMenu->hbrBack) ? pMenu->hbrBack : SYSHBR(MENUHBR);
	//
	...
	//		//
	//		//hbrT = (HBRUSH)SelectObject(hdc, SYSHBR(MENUHBR));
	//		//PatBlt(hdc, cxFrame, yTop, pMenu->cxMenu, pMenu->cyMenu, PATCOPY);
	//
	//		//
	//		// Draw border under menu in proper BORDER color
	//		//
	//		//(HBRUSH)SelectObject(hdc, (TestWF(pwnd, WEFEDGEMASK) && !TestWF(pwnd, WFOLDUI))? SYSHBR(3DFACE) : SYSHBR(WINDOWFRAME));
	//		//PatBlt(hdc, cxFrame, yTop + pMenu->cyMenu, pMenu->cxMenu, SYSMET(CYBORDER), PATCOPY);
	//
	//
	//	}
	//
	...
	//	 */
	//
	//	hbrT = (HBRUSH)SelectObject(hdc, (TestWF(pwnd, WEFEDGEMASK) && !TestWF(pwnd, WFOLDUI)) ? SYSHBR(3DFACE) : SYSHBR(WINDOWFRAME));
	//	xxxMenuDraw(hdc, pMenu);
	//	(HBRUSH)SelectObject(hdc, hbrT);
	//
	//	if (fClipped)
	//	{
	//#ifdef LATER
	...
	PMENU pmenu)
{
	...
	// 如果菜单的高度超出屏幕高度，可能出现上下滚动箭头，当出现箭头时，
	// 重新计算第一个Menu Item并重设视口
	if (pmenu->dwArrowsOn != MSA_OFF)
	{
		pItem = MNGetToppItem(pmenu);
		SetViewportOrgEx(hdc, ptOrg.x, ptOrg.y - ((int)pItem->yItem), NULL);
		i = pmenu->iTop;
	}
	else
	{
	...
	{
		// 通过为菜单项设置MFT_MENUBARBREAK类型，可以放置菜单项在新行上(适用于菜单栏)
		// 或在新列内(适用于下拉菜单，子菜单,快捷菜单).
		// 对于下拉菜单，子菜单，或快捷菜单，用垂直线分隔以前的列和新的列
		// 菜单项还有一个MFT_MENUBREAK类型，与MFT_MENUBARBREAK类似，只不过不用垂直线
		// 分隔以前的列和新的列
	...
	BOOL fOn)
{
	...
	lStyle = GetWindowLong(pwndNotify, GWL_STYLE);
	if (!TestMF(pmenu, MFISPOPUP) && lStyle & WS_MINIMIZE)
	{
	...
	if (fOn && (ppopupmenu != NULL) && (pmenu->dwArrowsOn != MSA_OFF))
	{
		...
		{
			pWalk = MNGetToppItem(pmenu);
			dy = pItem->yItem + pItem->cyItem - pWalk->yItem - pmenu->cyMenu;
			iNewTop = pmenu->iTop;
...
NewTop:
	...
	{
		hdc = GetWindowDC(pwnd);
		lStyle = GetWindowLong(pwnd, GWL_STYLE);
		if (lStyle & WS_SIZEBOX && !fSysMenuIcon)
		{
			...
			CIUIRect rcWindow;
			::GetWindowRect(pwnd, rcWindow);
			int xMenuMax = (rcWindow.right - rcWindow.left) - SYSMET(CXSIZEFRAME);

			if (rcItem.right > xMenuMax ||
				rcItem.bottom > ((rcWindow.bottom -	rcWindow.top) - SYSMET(CYSIZEFRAME)))
			{
	...
	hOldFont = (HFONT)SelectObject(hdc, ghMenuFont);
	GetViewportOrgEx(hdc, &ptOrg);
			...
			|| (TestMF(pmenu, MFISPOPUP))))
	{
		...
		if (pmenu->dwArrowsOn != MSA_OFF)
		{
			SetViewportOrgEx(hdc, ptOrg.x, ptOrg.y - ((int)MNGetToppItem(pmenu)->yItem), NULL);
		}
			...
			&& !TestMFT(pItem, MFT_OWNERDRAW))
		{
			 ...
			 * around with hbrBack
			 */
			int iBkSave = SetBkMode(hdc, TRANSPARENT);
	...
	HFONT   hfont)
{
	...
	//	FillRect(hdc, lprc, SYSHBR(MENUHBR));
	//
	//	/*
	//	 *  Finally, draw the menu itself.
	//	 */
...
\***************************************************************************/
void xxxDrawMenuBarUnderlines(HWND pwnd, BOOL fShow)
{
	// 	HDC hdc;
	// 	PMENU pmenu;
	// 	PITEM pitem;
	...
	// 	pitem = (PITEM)pmenu->rgItems;
	//
	// 	hdc = GetDCEx(pwnd, NULL, DCX_WINDOW | DCX_USESTYLE | DCX_CACHE);
	//
	// 	// select in the correct brush and font
	//
	// 	(HFONT)SelectObject(hdc, ghMenuFont);
	//
	...
	// 		else
	// 		{
	// 			SetViewportOrgEx(hdc, pitem->xItem, pitem->yItem, NULL);
	// 		}
	//
	...
	// 		{
	// 			(HBRUSH)SelectObject(hdc, SYSHBR(HOTLIGHT));
	// 		}
	// 		else
	// 		{
	// 			(HBRUSH)SelectObject(hdc, hbr);
	// 		}
	//
	// 		if (TestMFT(pitem, MFT_OWNERDRAW))
	// 		{
	...
	// 					SetTextColor(hdc, SYSRGB(MENUHBR));
	// 				}
