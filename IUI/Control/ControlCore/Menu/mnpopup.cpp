...
UINT IUI::gwinOldAppHackoMaticFlags; // Flags for doing special things for
// winold app
	...
	CONST TPMPARAMS *lpTpm)
{
	...
	ptiOwner = ptiCurrent;//GETPTI(pwndOwner);
	...
	//			KillTimer(ppopupMenuHierarchy->spwndPopupMenu, IDSYS_MNSHOW);
	...
	{
		fButtonDown = (GetKeyState(VK_RBUTTON) & 0x8000) != 0;
	}
	else
	{
		fButtonDown = (GetKeyState(VK_LBUTTON) & 0x8000) != 0;
	}
	 ...
	 */
	WNDCLASS wc;
	wc.lpfnWndProc     = xxxMenuWindowProc;
	...
	wc.cbWndExtra      = sizeof(POPUPMENU);// 这样，SetWindowLong(HWND, 0, void *)才能设置正确，0一般用来存放控件变量
	wc.cbClsExtra      = 0;
	...
	pwndHierarchy = ::CreateWindowEx(
			...
			pwndOwner,
			NULL,
			(HINSTANCE)GetWindowLong(pwndOwner, GWL_HINSTANCE),
			NULL);
	...
	{
		::SendMessage(pwndOwner, WM_ENTERMENULOOP,
	...
	{
		if (!SUCCEEDED(xxxClientRegisterDragDrop(pwndHierarchy)))
		{
	...
	if (!ppopupMenuHierarchy->fNoNotify)
	{
		...
		::SendMessage(ppopupMenuHierarchy->spwndNotify, WM_INITMENUPOPUP,
			(WPARAM)pMenu, MAKELONG(0, (ppopupMenuHierarchy->fIsSysMenu ? 1 : 0)));
...
AbortTrackPopupMenuEx:
		...
		{
			::SendMessage(pwndOwner, WM_EXITMENULOOP, TRUE, 0L);
		}
	...
	pt.x = x;
	pt.y = y;
	pMonitor = MonitorFromPoint(pt, MONITOR_DEFAULTTONEAREST);
	...
	HMONITOR    pMonitor)
{
	...
	if (prcExclude != NULL)
	{
		...
		IntersectRect(&rcExclude, &rcExclude, &mi.rcMonitor);
	}
	else
	{
