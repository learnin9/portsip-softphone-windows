...
PMENU xxxGetInitMenuParam(HWND pwndMenu, BOOL *lpfSystem);
/***************************************************************************\
	...
	PMENU pmenusys)
{
	...
	LONG lExStyle = GetWindowLong(pwnd, GWL_EXSTYLE);
	...
	if (!(lStyle & WS_MINIMIZE))
	{
		...
		cBorders = 4;//GetWindowBorders(lStyle, lExStyle, TRUE, FALSE);
...
\***************************************************************************/
void IUI::MNFlushDestroyedPopups(PPOPUPMENU ppopupmenu, BOOL fUnlock)
{
...
*
\***************************************************************************/
PPOPUPMENU IUI::MNAllocPopup(BOOL fForceAlloc)
{
	PPOPUPMENU ppm;
	...
	{
		ppm = (PPOPUPMENU)malloc(sizeof(POPUPMENU));//UserAllocPoolWithQuota(sizeof(POPUPMENU), TAG_POPUPMENU);
	}
...
\***************************************************************************/
void IUI::xxxMNEndMenuState(BOOL fFreePopup)
{
...
BOOL MNCreateAnimationBitmap(PMENUSTATE pMenuState, UINT cx, UINT cy)
{
	...
	::SelectObject(pMenuState->hdcAni, hbm);
...
*
\***************************************************************************/
void IUI::MNDestroyAnimationBitmap(PMENUSTATE pMenuState)
{
...
{
	HDC hDC = GetDC(GetDesktopWindow());
	pMenuState->hdcAni = CreateCompatibleDC(hDC);
	if (pMenuState->hdcAni == NULL)
	{
...
\***************************************************************************/
BOOL IUI::xxxUnlockMenuState(PMENUSTATE pMenuState)
{
...
\***************************************************************************/
PMENUSTATE IUI::xxxMNAllocMenuState(PTHREADINFO ptiCurrent, PTHREADINFO ptiNotify, PPOPUPMENU ppopupmenuRoot)
{
	BOOL fAllocate;
	PMENUSTATE pMenuState = NULL;
	 ...
	 */
	fAllocate = TRUE;//TEST_PUDF(PUDF_MENUSTATEINUSE);
	if (fAllocate)
	{
		pMenuState = (PMENUSTATE)malloc(sizeof(MENUSTATE));//UserAllocPoolWithQuota(sizeof(MENUSTATE), TAG_MENUSTATE);
		if (pMenuState == NULL)
		{
			return NULL;
		}
...
\***************************************************************************/
PMENUSTATE xxxMNStartMenuState(HWND pwnd, DWORD cmd, LPARAM lParam)
{
	PPOPUPMENU ppopupmenu;
	PTHREADINFO ptiCurrent = NULL;
	PTHREADINFO ptiNotify = NULL;
	PMENUSTATE pMenuState;
	//TL tlpwnd;
	HWND pwndT;
	...
	if (!(lStyle & WS_CHILD) /*&& (pwnd->spmenu != NULL)*/)
	{
		goto hasmenu;
	}
...
hasmenu:
	...
	if (ptiNotify != ptiCurrent)
	{
		...
		::PostMessage(pwnd, WM_SYSCOMMAND, cmd, lParam);
		return NULL;
	}
	...
	::SendMessage(pwnd, WM_ENTERMENULOOP, 0, 0L);
	ThreadUnlock(&tlpwnd);
	...
	int mn)
{
	...
	pMenuState->fButtonDown =
		pMenuState->fButtonAlwaysDown = ((GetKeyState(VK_LBUTTON) & 0x8000) != 0);

	xxxMNSetCapture(ppopupmenu);

	::SendMessage(pwndMenu, WM_SETCURSOR, (WPARAM)pwndMenu,
	...
	if (!ppopupmenu->fNoNotify)
	{
		...
		::SendMessage(pwndMenu, WM_INITMENU, (WPARAM)pMenu, 0L);
	}
...
PMENU xxxGetInitMenuParam(HWND pwndMenu, BOOL *lpfSystem)
{
	...
	LONG lStyle = GetWindowLong(pwndMenu, GWL_STYLE);
	if (lStyle & WS_MINIMIZE ||
		lStyle & WS_CHILD ||
		(GetMenu(pwndMenu) == NULL)/* ||
		!pwndMenu->spmenu->cItems*/)
	{
		...
		return NULL;//(xxxGetSysMenu(pwndMenu, FALSE));
	}
	else
	{
		if (lpfSystem != NULL)
