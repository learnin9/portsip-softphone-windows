...
BOOL xxxSetLPITEMInfo(PMENU pMenu, PITEM pItem, LPMENUITEMINFOW lpmii, CIUIStringW pstr);
	...
	LPCWSTR pstrItem)
{
	...
	if (pMenu->cItems >= pMenu->cAlloced)
	{
		if (pMenu->rgItems)
		{
			...
			//		DTAG_MENUITEM);
			pNewItems = (PITEM)malloc((pMenu->cAlloced + CMENUITEMALLOC) * sizeof(ITEM));
		...
		else
		{
			...
			pNewItems = (PITEM)malloc(sizeof(ITEM) * CMENUITEMALLOC);
		}
	...
	BOOL fDeleteMenu)
{
	...
	else
	{
		...
		if ((pMenu->cAlloced - pMenu->cItems) >= CMENUITEMDEALLOC - 1)
		{
			...
			//		DTAG_MENUITEM);
			pNewItems = (PITEM)malloc((pMenu->cAlloced - CMENUITEMDEALLOC) * sizeof(ITEM));
			if (pNewItems != NULL)
			{
	...
	CIUIStringW pstrItem)
{
	...
	if (lpmii->fMask & MIIM_STRING)
	{
		if (!pstrItem.IsEmpty())
		{
			...
			int nAllocLen = pstrItem.GetLength() + 1;
			hstr = new WCHAR[nAllocLen];//HANDLE)malloc(pstrItem.GetLength() + sizeof(UNICODE_NULL));
			if (hstr == NULL)
			{
				return FALSE;
			}
			ZeroMemory(hstr, nAllocLen * sizeof(WCHAR));

			//try
			{
				RtlCopyMemory(hstr, (LPCWSTR)pstrItem, pstrItem.GetLength() * sizeof(WCHAR));
			}
	...
	if (lpmii->fMask & MIIM_SUBMENU)
	{
		...
		{
			pSubMenu = HIUIMENUFromHMENU(lpmii->hSubMenu);//ValidateHmenu(lpmii->hSubMenu);
		}
...
FailInsertion:
				 ...
				 */
				{
					PMENULIST pMenuList = new MENULIST;//DesktopAlloc(pMenu->head.rpdesk, sizeof(MENULIST), DTAG_MENUITEM);
					if (!pMenuList)
					{
						goto FailInsertion;
					}
...
\***************************************************************************/
PPOPUPMENU IUI::MNGetPopupFromMenu(PMENU pMenu, PMENUSTATE *ppMenuState)
{
	PPOPUPMENU  ppopup;
	PMENUSTATE pMenuState;
...
*  07-23-96 GerardoB - Added header & fixed up for 5.0
\***************************************************************************/
void IUI::xxxMNUpdateShownMenu(PPOPUPMENU ppopup, PITEM pItem, UINT uFlags)
{
	RECT rc;
	...
	if (pMenu->cxMenu == 0)
	{
		...
		UserAssert(uFlags != 0);
		dwSize = (DWORD)::SendMessage(pwnd, MN_SIZEWINDOW, uFlags, 0L);
		uFlags &= ~MNUS_DRAWFRAME;
		/*
		...
		if (pItem != NULL)
		{
			...
			else
			{
				...
				if (pMenu->dwArrowsOn != MSA_OFF)
				{
					...
					::GetClientRect(pwnd, &rcScroll);
				}

				rc.top = rcScroll.top = pItem->yItem - MNGetToppItem(pMenu)->yItem;
				if ((rc.top >= 0) && (rc.top < (int)pMenu->cyMenu))
				{
					ScrollWindowEx(pwnd, 0, rc.bottom - cySave, &rcScroll, &rc, NULL, NULL, SW_INVALIDATE | SW_ERASE);
				}
...
InvalidateAll:
		...
		{
			::SetWindowPos(pwnd, NULL, 0, 0, 0, 0,
