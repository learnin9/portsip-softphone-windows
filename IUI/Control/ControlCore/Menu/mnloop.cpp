...
BOOL IUI::xxxHandleMenuMessages(
	LPMSG lpmsg,
...
HandleButtonDown:
		 ...
		 */
		pMenuState->mnFocus = MOUSEHOLD;
		// 坐标是相对Notify Window的。也就是相对捕获了鼠标的窗口。
		pMenuState->ptMouseLast.x = GET_X_LPARAM(lParam);
		pMenuState->ptMouseLast.y =  GET_Y_LPARAM(lParam);

		// Scott. 转成相对屏幕坐标
		::ClientToScreen(ppopupmenu->spwndNotify, &pMenuState->ptMouseLast);
		lParam = MAKELPARAM(pMenuState->ptMouseLast.x, pMenuState->ptMouseLast.y);
		...
		if (fThreadLock)
		{
			ThreadLock((HWND)cmdHitArea, &tlpwndHitArea);
		}
...
CancelOut:
		...
		else
		{
			...
			::SendMessage((HWND)cmdHitArea, MN_BUTTONDOWN, cmdItem, 0L);
		}
	...
	case WM_NCMOUSEMOVE:
			...
			*(LPPOINT)&rc.right = pMenuState->ptButtonDown;
			...
			if (!PtInRect(&rc, pt))
			{
				...
				if (pwnd != NULL)
				{
					...
					PostMessage(pwnd, MN_DODRAGDROP, 0, 0);
				}
				else
				{
		...
		{
			// Scott add，转成相对于屏幕的坐标
			CIUIPoint ptScreen(lParam);
			::ClientToScreen(lpmsg->hwnd, &ptScreen);
			LPARAM l = MAKELPARAM(ptScreen.x, ptScreen.y);

			xxxMNMouseMove(ppopupmenu, pMenuState, MAKEPOINTS(l));
		}
		return TRUE;
	...
	case WM_NCRBUTTONUP:
		...
		if ((lpmsg->message == WM_RBUTTONUP) && !ppopupmenu->fNoNotify)
		{
			...
			if ((ppopupmenu->spwndActivePopup != NULL)
				&& (ppopupActive = GetPopupMenu(ppopupmenu->spwndActivePopup))
				&& MNIsItemSelected(ppopupActive))
			{
				//TL tlpwndNotify;
				ThreadLock(ppopupActive->spwndNotify, &tlpwndNotify);
				::SendMessage(ppopupActive->spwndNotify, WM_MENURBUTTONUP,
					ppopupActive->posSelectedItem, (LPARAM)ppopupActive->spmenu);
...
HandleButtonUp:
		...
		else
		{
			if ((cmdHitArea == MFMWFP_OFFMENU) && (cmdItem == 0))
			{
				if (!ppopupmenu->fFirstClick)
				{
					...
					::SendMessage(ppopupmenu->spwndPopupMenu, MN_CANCELMENUS, 0, 0);
		...
		else if ((cmdHitArea != MFMWFP_OFFMENU) && (cmdHitArea != MFMWFP_ALTMENU))
		{
			...
			// compat
			//
			::SendMessage((HWND)cmdHitArea, MN_BUTTONUP, (DWORD)cmdItem, lParam);
		}
		else
		{
			pMenuState->fButtonDown =
...
Unlock:
	...
	case WM_NCLBUTTONDBLCLK:
		...
		else
		{
			...
			ThreadLock((HWND)cmdHitArea, &tlpwndHitArea);
			::SendMessage((HWND)cmdHitArea, MN_DBLCLK,
				(DWORD)cmdItem, 0L);
			ThreadUnlock(&tlpwndHitArea);
		}
		return TRUE;
	...
	case WM_SYSKEYDOWN:
		...
		case VK_F1:
			if (ppopupmenu->spwndActivePopup)
			{
				...
				::SendMessage(ppopupmenu->spwndActivePopup, lpmsg->message,
					ch, 0L);
				ThreadUnlock(&tlpwndT);
			}
			else
...
\***************************************************************************/
void IUI::xxxEndMenuLoop(PMENUSTATE pMenuState, PPOPUPMENU ppopupmenu)
{
	...
	//{
	//	HWND pwndNotify = ppopupmenu->spwndNotify;
	//	PTHREADINFO pti = GETPTI(pwndNotify);
	//	BOOL fFrameOn = (pti->pq == gpqForeground) && (pti->pq->spwndActive == pwndNotify);
	//	TL tlpwndNotify;
...
int ParseMessage(LPCSTR lpszBuf, UINT *puMsg, CIUIString *pstrMsg)
{
	if (lpszBuf == NULL || puMsg == NULL || pstrMsg == NULL)
	{
		return -1;
	}

	CIUIStringA strBuf = lpszBuf;
	strBuf.TrimLeft();
	strBuf.TrimRight();

	// 如果不包含"#define"，或者"#define"不在行首，则为无效宏定义
	int nPos = strBuf.Find("#define");
	if (nPos != 0)
	{
		return -2;
	}
	...
	// 删除"#define"
	strBuf.Delete(nPos, 7);

	// 删除"#define"与宏之间的空白字符
	strBuf.TrimLeft();

	// 查找宏结束位置
	int nMacroEnd = strBuf.FindOneOf(" \t");
	if (nMacroEnd <= 0)
	{
		// 不是有效的消息宏定义
		return -3;
	...
	// 得取宏名字
	CIUIStringA strMacro;
	strMacro = strBuf.Left(nMacroEnd);

#if 0
	// 黑名单
	// 以下字符串开头的不是消息宏
	std::vector<CStringA> vFilter;
	vFilter.push_back("SPI_");
	vFilter.push_back("CTLCOLOR_");
	vFilter.push_back("COLOR_");
	vFilter.push_back("IS_");
	vFilter.push_back("WINUSERAPI");
	vFilter.push_back("WINABLEAPI");
	vFilter.push_back("WINVER");
	vFilter.push_back("MAKEINTRESOURCE");
	vFilter.push_back("RT_");
	vFilter.push_back("DIFFERENCE");

	BOOL bNotMsg = FALSE;
	std::vector<CStringA>::iterator it = vFilter.begin();
	for (; it != vFilter.end(); ++it)
	{
		CStringA *pFilter = &(*it);

		if (strMacro.Left(pFilter->GetLength()) == *pFilter)
		{
			bNotMsg = TRUE;
			break;
		}
	...
	// 白名单
	// 只有以下字符串开头才是消息宏
	std::vector<CIUIStringA> vFilter;
	vFilter.push_back("WM_");
	vFilter.push_back("LB_");

	BOOL bMsg = FALSE;
	std::vector<CIUIStringA>::iterator it = vFilter.begin();
	for (; it != vFilter.end(); ++it)
	{
		CIUIStringA *pFilter = &(*it);
	...
	// 如果消息值包含"0x"，则认为是16进制值，否则为10进制
	UINT nMacroValue = 0;
	if (strBuf.Left(2) == "0x")
	{
		nMacroValue = strtoul(strBuf, NULL, 16);
	}
	else
	{
		nMacroValue = strtoul(strBuf, NULL, 10);
	}
	...
	USES_CONVERSION;
	*puMsg = nMacroValue;
	*pstrMsg = A2CT(strMacro);

	return 0;
}

class CMessageTranslate
{
public:
	CMessageTranslate()
	{
...
public:
	// winuser.h完整路径
	int SetSource(LPCTSTR lpszMessageFile)
	{
		// 以ANSI方式读文件
		USES_CONVERSION;
		FILE *fp = fopen(T2CA(lpszMessageFile), "rb");
		if (fp == NULL)
		{
			return -1;
		}
		...
		do
		{
			...
			{
				CIUIStringA strLine;
				fgets(strLine.GetBufferSetLength(MAX_PATH), MAX_PATH, fp);

				UINT uMsg = 0;
				CIUIString strMsg;
				int nRet = ParseMessage(strLine, &uMsg, &strMsg);
				if (nRet == 0)
				{
					m_mapMsg[uMsg] = strMsg;
				}
			}
		...
		// 未定义消息
		m_mapMsg[0x01AE] = _T("LBCB_STARTTRACK");
		m_mapMsg[0x01AF] = _T("LBCB_ENDTRACK");
		m_mapMsg[0x01A3] = _T("LBCB_CARETON");
		m_mapMsg[0x01A4] = _T("LBCB_CARETOFF");

		m_mapMsg[0x022A] = _T("WM_DROPOBJECT");
		m_mapMsg[0x022B] = _T("WM_QUERYDROPOBJECT");
		m_mapMsg[0x022C] = _T("WM_BEGINDRAG");
		m_mapMsg[0x022D] = _T("WM_DRAGLOOP");
		m_mapMsg[0x022E] = _T("WM_DRAGSELECT");
		m_mapMsg[0x022F] = _T("WM_DRAGMOVE");
		m_mapMsg[0x0037] = _T("WM_QUERYDRAGICON");
		m_mapMsg[0x0233] = _T("WM_DROPFILES");

		/* Title bar control messages */
		m_mapMsg[0x01e3] = _T("TBM_SETHILITE");
		m_mapMsg[0x01e4] = _T("TBM_QUERYHILITE");
		m_mapMsg[0x01e5] = _T("TBM_TRACKMOVE");

		return 0;
	}

	CIUIString GetMessageString(UINT uMsg)
	{
		std::map<UINT, CIUIString>::iterator it = m_mapMsg.find(uMsg);
		if (it == m_mapMsg.end())
		{
			return _T("");
		}
...
protected:
	std::map<UINT, CIUIString> m_mapMsg;
};

CMessageTranslate g_mt;
	...
	BOOL fDblClk)
{
	...
	if (!b)
	{
		g_mt.SetSource(_T("C:\\Program Files (x86)\\Microsoft SDKs\\Windows\\v7.0A\\include\\winuser.h"));
		b = true;
	}
	...
	pMenuState->ptMouseLast.x = pt.x;//pti->ptLast.x;
	pMenuState->ptMouseLast.y = pt.y;//pti->ptLast.y;
	...
	{
		if (GetKeyState(((ppopupmenu->fRightButton) ?
					VK_RBUTTON : VK_LBUTTON)) >= 0)
		{
			...
			if (!ppopupmenu->fNoNotify)
			{
				...
				::SendMessage(ppopupmenu->spwndNotify, WM_EXITMENULOOP,
		...
		msg.lParam = lParam;
		msg.hwnd = ppopupmenu->spwndPopupMenu;
	...
	while (pMenuState->fInsideMenuLoop)
	{
		 ...
		 */
		// 原来的代码是BOOL fPeek = PeekMessage(&msg, NULL, 0, 0, PM_NOYIELD | PM_NOREMOVE);
		// 使用带PM_NOREMOVE参数的PeekMessage会导致消息不被删除，拿到的永远是WM_MOUSELEAVE消息
		// 应该在某个地方，会删除队列中的消息，但没有找到在何处，故使用GetMessage代替。
		BOOL fGot = GetMessage(&msg, NULL, 0, 0);
		//BOOL fPeek = PeekMessage(&msg, NULL, 0, 0, PM_NOYIELD | PM_NOREMOVE);
...
NoMsg:
			...
			UserAssert((ppopupmenu->spwndActivePopup == NULL)
				|| (::IsWindowVisible(ppopupmenu->spwndActivePopup)));
			...
			if (fSendIdle)
			{
				if (ppopupmenu->spwndNotify != NULL)
				{
					...
					::SendMessage(ppopupmenu->spwndNotify, WM_ENTERIDLE, MSGF_MENU,
						(LPARAM)ppopupmenu->spwndActivePopup);
					ThreadUnlock(&tlpwndT);
				}
				fSendIdle = FALSE;
			}
