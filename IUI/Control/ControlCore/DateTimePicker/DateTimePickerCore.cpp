...
// DATEPICK
LRESULT DPCreateHandler(DATEPICK *pdp, HWLWND hwnd, LPCREATESTRUCT lpcs);
...
void DPDestroyHandler(HWLWND hwnd, DATEPICK *pdp, WPARAM wParam, LPARAM lParam);
void DPHandleSetFont(DATEPICK *pdp, HFONT hfont, BOOL fRedraw);
int DPPaint(DATEPICK *pdp, HDC hdc, LPCRECT lprcPaint);
...
void DPDrawDropdownButton(DATEPICK *pdp, HDC hdc, BOOL fPressed, BOOL bCtrlDC);
...
LRESULT CALLBACK MonthCalWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    //			IUI::InvalidateRect(pmc->hwnd, NULL);
    //		}
    //		break;
    //	}
    //
...
{
    pclr[MCSC_BACKGROUND] = IUIGetGloabelVar()->g_clrWindow;
    pclr[MCSC_MONTHBK] = IUIGetGloabelVar()->g_clrWindow;
    pclr[MCSC_TEXT] = IUIGetGloabelVar()->g_clrWindowText;
    pclr[MCSC_TITLEBK] = GetSysColor(COLOR_ACTIVECAPTION);
    pclr[MCSC_TITLETEXT] = GetSysColor(COLOR_CAPTIONTEXT);
    //pclr[MCSC_TRAILINGTEXT] = IUIGetGloabelVar()->g_clrGrayText;
}
...
LRESULT MCOnStyleChanged(MONTHCAL *pmc, UINT gwl, LPSTYLESTRUCT pinfo)
{
    ...
    //	_ASSERT(!(changeFlags & (MCS_MULTISELECT | MCS_DAYSTATE | MCS_INVALIDBITS)));
...
void MCUpdateStartEndDates(MONTHCAL *pmc, SYSTEMTIME *pstStart)
{
    ...
    //	_ASSERT(0 == CmpDate(&pmc->stMonthFirst, &pmc->stMin));
    //}
    ...
    //	_ASSERT(0 == CmpDate(&pmc->stMonthLast, &pmc->stMax));
    //}
...
void SEGetTimeDateFormat(LPSUBEDIT pse, LPSUBEDITCONTROL psec, LPTSTR pszBuf, DWORD cchBuf)
{
    ...
    _ASSERT(cchBuf >= 2);    // We assume it can hold at least space and a null
...
int SECGetMaxEraLength(PCALENDARTYPE pct, HDC hdc, PSIZE psize)
{
    ...
    {
        LPCTSTR ptsz = (LPCTSTR)DPA_FastGetPtr(pct->hdpaEras, i);
...
void SECRecomputeSizing(LPSUBEDITCONTROL psec, LPRECT prc)
{
    ...
    hdc = GetDC(GetHostWnd(psec->pci->hwnd));
...
HaveWidth:
    ...
    SelectObject(hdc, hfontOrig);
    ReleaseDC(GetHostWnd(psec->pci->hwnd), hdc);
}
...
BOOL PASCAL SECParseFormat(DATEPICK *pdp, LPSUBEDITCONTROL psec, LPCTSTR szFormat)
{
    ...
    //
    if (pdp->fLocale &&
        (pdp->hwnd->ci.style & DTS_FORMATMASK) == DTS_SHORTDATECENTURYFORMAT)
    {
        fForceCentury = TRUE;
    }
...
SearchSomeMore:
...
#ifdef DEBUG
    {
        ...
        while (cse > 0)
        {
            ...
            StringCchPrintf(psz, (psz - sz) / sizeof(TCHAR), TEXT("[%s] "), pse->pv);
#else
    ...
    InvalidateRect(psec->pci->hwnd, NULL);

    // Changing the format also changes the window text.
    MyNotifyWinEvent(EVENT_OBJECT_NAMECHANGE, GetHostWnd(pdp->hwnd), OBJID_WINDOW, INDEXID_CONTAINER);
...
void InvalidateScrollRect(HWLWND hwnd, RECT *prc, int xScroll)
{
    RECT rc;
...
#undef YearAffected
        ...
        if (psec->iseCur >= 0)
        {
            ...
            {
                InvalidateRect(psec->pci->hwnd, &rc);
            }
        }
    }
}
...
UINT SECAdjustByType(DATEPICK *pdp, LPSUBEDIT psubed, UINT uNewValue)
{
    ...
    if (SE_YEARLIKE(psubed->id))
    {
        if (uNewValue < 100)
        {
            ...
            //if (!NT5_GetCalendarInfoA(pdp->sec.ct.lcid, pdp->sec.ct.calid,
            //	CAL_RETURN_NUMBER + CAL_ITWODIGITYEARMAX, NULL, 0, &dwMax2DigitYear))
            {
...
UINT SECIncrementEra(LPSUBEDITCONTROL psec, LPSUBEDIT pse, int delta)
{
    ...
    _ASSERT(pse->pval == &psec->st.wYear);
    ...
    for (i = 0; i < cEras; i++)
    {
        if (lstrcmp(rgch, (LPCWSTR)DPA_FastGetPtr(psec->ct.hdpaEras, i)) == 0)
        {
            goto FoundEra;
        }
    }
...
FoundEra:
    ...
    if (delta > 0) // Incrementing
    {
        ...
        uNewYear = StrToInt((PCWSTR)DPA_FastGetPtr(psec->ct.hdpaYears, i));
    }
    else
    {
        //
...
BOOL SECIsNumeric(LPSUBEDIT psubed)
{
    ...
    {
    case SE_ERA:
        return FALSE;           // g never
    case SE_YEAR:
        return TRUE;            // yyyy always digits
    case SE_YEARALT:
        return TRUE;            // yy always digits
    case SE_MONTH:
        return lstrlen(psubed->pv) <= 2; // MM yes, but not MMM
    case SE_MONTHALT:
        return lstrlen(psubed->pv) <= 4; // ddMM yes, but not ddMMM
    case SE_DAY:
        return TRUE;            // dd always digits
    case SE_MARK:
        return FALSE;           // tt never
    case SE_HOUR:
        return TRUE;            // hh always digits
    case SE_MINUTE:
        return TRUE;            // mm always digits
    case SE_SECOND:
        return TRUE;            // ss always digits
    case SE_STATIC:
        return FALSE;           // static text
    case SE_APP:
        return FALSE;           // app's job to format this
    }
    return FALSE;
}
...
void SECDrawSubedits(HDC hdc, LPSUBEDITCONTROL psec, BOOL fFocus, BOOL fEnabled)
{
    ...
    int i, iseCur;
    LPTSTR sz;
    TCHAR szTmp[DTP_FORMATLENGTH] = { 0 };
    LPSUBEDIT psubed;
    ...
    SetBkColor(hdc, IUIGetGloabelVar()->g_clrHighlight);
    ...
    for (i = 0, psubed = psec->pse; i < psec->cse; i++, psubed++)
    {
        ...
        {
            SetBkMode(hdc, TRANSPARENT);
            SetTextColor(hdc, IUIGetGloabelVar()->g_clrGrayText);
        }
        else if (iseCur == i)
        {
            SetBkMode(hdc, OPAQUE);
            SetTextColor(hdc, IUIGetGloabelVar()->g_clrHighlightText);
        }
        else
        {
            SetBkMode(hdc, TRANSPARENT);
        ...
        if (i == psec->iseCur && psubed->cchEdit != 0)
        {
            ...
            {
                TCHAR szFormat[10];
                StringCchPrintf(szFormat, 10, TEXT("%%0%dd"), psubed->cchEdit);
                StringCchPrintf(szTmp, DTP_FORMATLENGTH, szFormat, psubed->valEdit);
                sz = szTmp;
            }
            else
            {
...
BOOL SECEdit(DATEPICK *pdp, LPTSTR szOutput, int cchOutput)
{
    ...
    hwndEdit = CreateWindowEx(0, TEXT("EDIT"), szBuf, WS_CHILD | ES_AUTOHSCROLL,
            ...
            GetHostWnd(psec->pci->hwnd), NULL, hIns, NULL);

    if (hwndEdit)
    {
        RECT rcEdit = psec->rc;

        ClientToScreen(psec->pci->hwnd, &rcEdit); // ClientToScreen
...
// 日期时间控件窗口过程（假控件）
LRESULT WINAPI IUI::DateTimePicker_WndProc(HWLWND hWnd, RoutedEventArgs *pe)
{
    DATEPICK *pdp = (DATEPICK *)GetMember(hWnd);
    LRESULT  lres = 0;
    ...
    if (hWnd == pe->hOriginalSender)
    {
        if (RS_BUBBLE == pe->eRoutingStrategy)
        {
            if (NULL == pdp)
            {
                if (WM_NCCREATE == message)
                {
                    ...
                    pdp = new DATEPICK;
                    pdp->hwnd = hWnd;
                    pdp->hwnd->ci.hwndParent = GetParent(hWnd);
            ...
            {
            case WM_CREATE:
                return DPCreateHandler(pdp, pdp->hwnd, (LPCREATESTRUCT)lParam);

            case WM_DESTROY:
                DPDestroyHandler(pdp->hwnd, pdp, wParam, lParam);
                return 0;
            ...
            // 与定向事件处理中的DTMP_WINDOWPOSCHANGED代码完全一样
            case WM_SIZE:
            {
                RECT rc;
                ...
                IUI::InvalidateRect(pdp->hwnd, NULL);
            }
            return 0;
        ...
        else if (RS_DIRECT == pe->eRoutingStrategy)
        {
            ...
            case WM_PRINTCLIENT:
            case WM_PAINT:
                DPPaint(pdp, (HDC)wParam, LPCRECT(lParam));
                return 0;

            // 与冒泡事件中WM_SIZE的处理完全一样
    ...
    case WM_NOTIFY:
        ...
        case UDN_DELTAPOS:
            if ((int)wParam == DATEPICK_UPDOWN)
            {
                ...
                if (!pdp->fFocus)
                {
                    IUI::SetFocus(GetHostWnd(pdp->hwnd), pdp->hwnd);
                }
    ...
    case WM_SETFOCUS:
    {
        ...
        if (BOOLIFY(fGotFocus) != BOOLIFY(pdp->fFocus))
        {
            ...
            if (pdp->sec.iseCur >= 0)
            {
                InvalidateScrollRect(pdp->hwnd, &pdp->sec.pse[pdp->sec.iseCur].rc, pdp->sec.xScroll);
            }
            else if (DatePick_ShowCheck(pdp))
            {
                pdp->fCheckFocus = (WORD)fGotFocus;
                IUI::InvalidateRect(pdp->hwnd, &pdp->rcCheck);
            }
            ...
            CCSendNotify(&pdp->hwnd->ci, (fGotFocus ? NM_SETFOCUS : NM_KILLFOCUS), &nmhdr);
        }
    ...
    case WM_NOTIFYFORMAT:
        return CIHandleNotifyFormat(&pdp->hwnd->ci, lParam);
        break;
    ...
    case WM_GETTEXTLENGTH:
    {
        ...
        {
            LPTSTR sz = SECFormatSubed(&pdp->sec, psubed, szTmp, ARRAYSIZE(szTmp));
            UINT nLen = lstrlen(sz);
    ...
    //		{
    //			pwszFormat = ProduceWFromA(pdp->hwnd->ci.uiCodePage, pszFormat);
    //		}
    //
    ...
    default:
        if (CCWndProc(&pdp->hwnd->ci, message, wParam, lParam, &lres))
        {
            return lres;
        }
...
LRESULT DPCreateHandler(DATEPICK *pdp, HWLWND hwnd, LPCREATESTRUCT lpcs)
{
    ...
    // Initialize our data.
    //CIInitialize(&pdp->hwnd->ci, hwnd, lpcs);

    //if (pdp->hwnd->ci.style & DTS_INVALIDBITS)
    //{
    //	return (-1);
    //}

    HINSTANCE hInst = IUIGetInstanceHandle();

    if (pdp->hwnd->ci.style & DTS_UPDOWN)
    {
        pdp->fUseUpDown = TRUE;

        pdp->hwndUD = CreateWindowEx(NULL, 0, IUI_SPINBUTTON, NULL,
                WS_CHILD | WS_VISIBLE | (pdp->hwnd->ci.style & WS_DISABLED),
                CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, GetHostWnd(hwnd),
                hwnd, XNAME_DATETIME_SPIN, hInst, NULL);

        // 使用默认属性初始化
        CSpinButtonProp spinProp;
        SpinButton_BindStyle(pdp->hwndUD, &spinProp);
    }
    if (DatePick_ShowCheck(pdp))
    {
    ...
    pdp->fEnabled = !(pdp->hwnd->ci.style & WS_DISABLED);
    ...
    {
        if (pdp->hwnd->ci.style & DTS_TIMEFORMATONLY)
        {
...
LRESULT DPOnStyleChanging(DATEPICK *pdp, UINT gwl, LPSTYLESTRUCT pinfo)
{
    if (gwl == GWL_STYLE)
    {
        DWORD changeFlags = pdp->hwnd->ci.style ^ pinfo->styleNew;
...
LRESULT DPOnStyleChanged(DATEPICK *pdp, UINT gwl, LPSTYLESTRUCT pinfo)
{
    if (gwl == GWL_STYLE)
    {
        ...
        _ASSERT(!(changeFlags & (DTS_UPDOWN | DTS_SHOWNONE)));

        pdp->hwnd->ci.style = pinfo->styleNew;
        ...
        if (changeFlags & (WS_BORDER | WS_CAPTION | WS_THICKFRAME))
        {
            ...
            // but not until after this message is handled
            // so post ourself a message.
            SendMessage(pdp->hwnd, DTMP_WINDOWPOSCHANGED, 0, 0);
        }
...
void DPHandleLocaleChange(DATEPICK *pdp)
{
    ...
    if (pdp->fLocale)
    {
        TCHAR szFormat[DTP_FORMATLENGTH] = { 0 };

        switch (pdp->hwnd->ci.style & DTS_FORMATMASK)
        {
...
void DPHandleSetFont(DATEPICK *pdp, HFONT hfont, BOOL fRedraw)
{
    ...
    pdp->hwnd->ci.uiCodePage = GetCodePageForFont(hfont);
...
int DateTimePicker_DrawBorder(DATEPICK *pdp, HDC hDC)
{
    _ASSERT(IsWindow(pdp->hwnd));

    if (!IsDrawBorderColor(pdp->hwnd))
    {
        return 1;
    }

    // Prepare state index
    CONTROL_STATE eButtonState = GetControlState(pdp->hwnd);
    if (!IsWindowEnabled(pdp->hwnd))
    {
    ...
    CIUIRect rcClient;
    GetClientRect(pdp->hwnd, rcClient);

    int nStateIndex = GetButtonStateIndex(eButtonState);
    COLORREF crBorder = RGB(128, 128, 128);// pdp->m_crBorder[nStateIndex];
...
int DP_DrawBkColor(DATEPICK *pdp, HDC hdc, LPCRECT lprcPaint)
{
    _ASSERT(IsWindow(pdp->hwnd));

    if (!IsDrawBkColor(pdp->hwnd))
    {
        return 1;
    }
    ...
    FillSolidRect(hdc, rcClient, pdp->m_crBk[nStateIndex],
        IsUpdateLayeredWindow(GetHostWnd(pdp->hwnd)));
...
int DPPaint(DATEPICK *pdp, HDC hdc, LPCRECT lprcPaint)
{
    ...
    // 绘制背景
    DP_DrawBkColor(pdp, hdc, lprcPaint);
    // 	Button_DrawBkImage(pdp, hdc, NULL);
    // 	Button_DrawFgImage(pdp, hdc);
    ...
    {
        DPDrawDropdownButton(pdp, hdc, FALSE, TRUE);
    }

    // 绘制边框
    DateTimePicker_DrawBorder(pdp, hdc);
...
void _RecomputeMonthCalRect(DATEPICK *pdp, LPRECT prcCal, LPRECT prcCalT)
{
    ...
    if (GetExStyle(pdp->hwnd) & WS_EX_TOPMOST)
    {
    ...
    if (rcCal.bottom > rcWorkArea.bottom)
    {
        ...
        IUI::ClientToScreen(pdp->hwnd, (LPPOINT)&rcT); // 2 ClientToScreen
...
void DPLBD_MonthCal(DATEPICK *pdp, BOOL fLButtonDown)
{
    ...
    if (fLButtonDown)
    {
        DPDrawDropdownButton(pdp, hdc, TRUE, FALSE);
    }

    rcT = pdp->rc;
    ClientToScreen(pdp->hwnd,  &rcT); //2 ClientToScreen

    rcBtn = pdp->rcBtn;
    ClientToScreen(pdp->hwnd, &rcBtn); //ClientToScreen
    ...
    hwndMC = ::CreateWindow(g_rgchMCName, NULL, WS_POPUP | WS_BORDER,
            ...
            GetHostWnd(pdp->hwnd), NULL, hIns, NULL);
    if (hwndMC == NULL)
    {
    ...
    _ASSERT(rcCalT.left == 0 && rcCalT.top == 0);
    ...
    CCSendNotify(&pdp->hwnd->ci, DTN_DROPDOWN, NULL);
    ...
    //
    {
        ...
        MoveWindow(hwndMC, rcCal.left, rcCal.top,
            /*rcCal.right - rcCal.left*/220, /*rcCal.bottom - rcCal.top*/200, FALSE);

        //#ifdef DEBUG
        //		if (GetAsyncKeyState(VK_CONTROL) < 0)
        //		{
        //			pmc->hwnd.ci.style |= MCS_MULTISELECT;
        //		}
        //#endif
    }
        ...
        if (fBtnActive)
        {
            ...
            case WM_MOUSEMOVE:
                ...
                else
                {
                    if (fBtnDown)
                    {
                        DPDrawDropdownButton(pdp, hdc, FALSE, FALSE);
                        fBtnDown = FALSE;
                    }
                    if (PtInRect(&rcCal, msg.pt))
                    {
    ...
    CCSendNotify(&pdp->hwnd->ci, DTN_CLOSEUP, NULL);

    pdp->hwndMC = NULL;
    DestroyWindow(hwndMC);
    ReleaseDC(GetHostWnd(pdp->hwnd), hdc);
}
...
void DPHandleSECEdit(DATEPICK *pdp)
{
    ...
    if (SECEdit(pdp, szBuf, ARRAYSIZE(szBuf)))
    {
        ...
        CCSendNotify(&pdp->hwnd->ci, DTN_USERSTRING, &nmdts.nmhdr);
...
LRESULT DPLButtonDown(DATEPICK *pdp, WPARAM wParam, LPARAM lParam)
{
    ...
    if (!fFocus)
    {
        SetFocus(GetHostWnd(pdp->hwnd), pdp->hwnd);
    }

    // 如果不是Spin按钮，显示弹出的日历窗口
    if (!pdp->fUseUpDown && PtInRect(&pdp->rcBtn, pt) && IsWindowVisible(pdp->hwnd))
    {
...
void DPNotifyDateChange(DATEPICK *pdp)
{
    ...
    //
    if (fChanged || pdp->hwnd->ci.iVersion < 5)
    {
        pdp->stPrev = nmdc.st;
        CCSendNotify(&pdp->hwnd->ci, DTN_DATETIMECHANGE, &nmdc.nmhdr);
    }
}
...
// bCtrlDC: 是否是控件DC，如果为FALSE，表示直接得到Host窗口DC，坐标相对于Host窗口；
// 如果为TRUE，hdc坐标相对于控件
void DPDrawDropdownButton(DATEPICK *pdp, HDC hdc, BOOL fPressed, BOOL bCtrlDC)
{
    UINT dfcs;
    ...
    RECT rcBtn = pdp->rcBtn;
    if (!bCtrlDC)
    {
        ClientToScreen(pdp->hwnd, &rcBtn);
        IUIScreenToClient(GetHostWnd(pdp->hwnd), &rcBtn);
    }

    DrawFrameControl(hdc, &rcBtn, DFC_SCROLL, dfcs);
}
