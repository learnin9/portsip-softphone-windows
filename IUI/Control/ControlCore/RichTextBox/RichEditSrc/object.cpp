	...
	LPCSTR lpstrContainerDoc)	//@parm	Container Object (doc) name
{
	...
	{
#ifndef MACPORT
		CStrInW szapp(lpstrContainerApp, CP_ACP);
		CStrInW szdoc(lpstrContainerDoc, CP_ACP);
#endif
#ifndef MACPORTREMOVE
		return pobjmgr->SetHostNames((LPWSTR)szapp, (LPWSTR)szdoc);
#else
		return pobjmgr->SetHostNames((LPOLESTR)lpstrContainerApp, (LPOLESTR)lpstrContainerDoc);
#endif
	}
	...
	LPDATAOBJECT *lplpdataobj)	//@parm where to put the data object
{
	...
	if (lpchrg)
	{
		...
		cpMost = lpchrg->cpMax;
		if (lpchrg->cpMax < 0 || lpchrg->cpMax > cchText)
		{
			cpMost = cchText;
		}
	}
