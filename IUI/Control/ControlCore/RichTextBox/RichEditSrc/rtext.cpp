...
// BUGBUG: the following constructor should just copy this rtp from the
// selection if it exists.  The compiler has given me all kinds of grief
// trying to do this, so for the moment, we walk out from cp = 0.  This
// could be real bad for fast typing in docs with lots of rich text.
 ...
 *
 *	@todo
 *		advance format run ptrs from 0 if target cp is closer to 0 than to
 *		cached cp (see CTxtPtr::AdvanceCp())
 */
