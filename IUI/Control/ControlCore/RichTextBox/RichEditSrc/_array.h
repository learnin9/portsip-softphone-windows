 ...
 *	never be used directly; use the type-safe template, <c CReArray>, instead.
 *
...
template <class ELEM>
// 原名为CArray，与MFC中的CArray冲突。 故改名为CReArray
class CReArray : public CArrayBase
{
	//@access Public Methods
public:

	CReArray()								//@cmember Constructor
...
extern "C"
{
	VOID	fumemmov(VOID *pvTo, VOID const *pvFrom, LONG cbMove);
	INT		fumemcmp(VOID const *pvA, VOID const *pvB, INT cb);
	VOID	fumemset(VOID *pv, INT iFill, INT cb);
}
