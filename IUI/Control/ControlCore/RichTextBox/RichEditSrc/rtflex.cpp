...
LONG CRTFRead::FillBuffer()
{
	...
	{
		// Leave 2 NULL chars so that else
		_pchRTFBuffer[0] = 0;			//  succeeds if we read < 2 chars
		_pchRTFBuffer[1] = 0;			//  and then call this routine again

	}
	else
	{
		_pchRTFBuffer[1] = _pchRTFCurrent[-1];	// Copy last 2 chars in case
		_pchRTFBuffer[0] = _pchRTFCurrent[-2];  //  we need to back up
	}
	_pchRTFCurrent = &_pchRTFBuffer[2];
	...
	_pchRTFEnd = &_pchRTFBuffer[2 + cchRead];		// Point the end
...
TOKEN CRTFRead::TokenGetHex()
{
	...
	{
		TRACEERRSZSC("TokenGetHex()", ecUnexpectedChar);
...
EC CRTFRead::SkipToEndOfGroup()
{
	...
	do
	{
		cchLeft = DiffPtrs(_pchRTFEnd, _pchRTFCurrent, BYTE);
		pch = _pchRTFCurrent;

		if (fEatChar)							// Wanted to eat a char last
		{
			//  time, but we were empty
			pch++;
			cchLeft--;
			fEatChar = FALSE;
		}
		...
		while (cchLeft--)
		{
			switch (*pch++)
			{
			case BSLASH:						// Skip next character
				if (cchLeft)
				{
					pch++;
					cchLeft--;
				}
				else							// Buffer is empty: set flag
				{
					fEatChar = TRUE;    // to eat a char next time
				}
				break;
...
TOKEN CRTFRead::TokenGetKeyword()
{
	...
	if (!IsAlphaChar(ach))							// Not alpha, i.e.,
	{
		...
		{
			//  special treatment
			if (TokenGetHex() == tokenError)	// Convert hex to char and
			{
				goto TokenError;    //  store result in _token
			}
		}
		else
		{
			pach = szSymbolKeywords;			// Check for other known
			while (ach != *pach && *pach)		//  symbols
			{
				pach++;
			}
			if (*pach)							// Found one
			{
				_token = tokenSymbol[pach - szSymbolKeywords];
				if (_token > 255)				// Token or larger Unicode
				{
					return _token;    //  value
				}
			}
	...
	BYTE ach)				// @parm First char of 8-bit text string
{
	...
	*pach++ = ach;
	while (cachText < cachTextMax - 1 &&
		(ach = GetChar()))
	{
		switch (ach)
		{
		case BSLASH:
			if (!(ach = GetChar()))				// Get char after '\'
			{
				goto error;
			}
			if (ach == '\'')					// Handle most frequent
			{
				//  case here
			...
			else
			{
				if ((ach == '\\') || (ach == '}') || (ach == '{'))
				{
					goto add;    //RTF control symbol in text
				}
				if (!UngetChar())					// Unget char after '\'
				{
					goto error;    // Fall thru to unget '\'
				}
			}
