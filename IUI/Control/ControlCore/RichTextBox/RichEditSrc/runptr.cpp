	...
	DWORD cp)			//@parm character position to move this RunPtr to
{
	...
	_iRun = 0;
	_ich = 0;
	// we'll always be going forward, so we can safely cast to a DWORD
	// here.
...
void CRunPtrBase::AdjustForward()
{
	...
	if( pRun && pRun->_cch == _ich )		// If at end of run, go to start
...
BOOL CRunPtrBase::IsValid() const
{
	...
	return _prgRun && _prgRun->Elem(_iRun);
}
