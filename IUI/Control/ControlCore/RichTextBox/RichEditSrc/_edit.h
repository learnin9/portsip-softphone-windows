...
public:
	...
	// A helper function
	DWORD GetTextLength() const
	{
		return _story.GetTextLength();
	}
	DWORD GetAdjustedTextLength();

	// Access to ActiveObject members

	IUnknown 		*GetPrivateIUnknown()
	{
		return &_unk;
	}
	CLightDTEngine *GetDTE()
	{
		return &_ldte;
	}
	...
	BOOL			fInplaceActive() const
	{
		return _fInPlaceActive;
	}
	BOOL			fHideSelection() const
	{
		return _fHideSelection;
	}
	BOOL			IsRich() const
	{
		return _fRich;
	}
	BOOL			fUsePassword() const
	{
		return _fUsePassword;
	}
	LONG			GetCpAccelerator() const
	{
		return _cpAccelerator;
	}
	HRESULT			UpdateAccelerator();
	void			CheckUnicode(LONG lStreamFormat);
	BOOL			fInOurHost() const
	{
		return _fInOurHost;
	}
	...
	// Document or Text String access
	CTxtStory 	*GetTxtStory(void)
	{
		return &_story;
	}

	// Text pointer, measurer and renderer factories
	CRchTxtPtr		*CreateTxtptr();
	CTxtStory		*CreateTxtStory();
	...
	DWORD		GetEventMask()
	{
		return _dwEventMask;   //@cmember Get event mask
	}
	...
	{
		return _phost->TxClientToScreen(lppt);
	}
	BOOL		TxScreenToClient(LPPOINT lppt)
	{
		return _phost->TxScreenToClient(lppt);
	}
	...
	HRESULT	 TxActivate(LONG *pOldState)
	{
		return _phost->TxActivate(pOldState);
	}
	HRESULT	 TxDeactivate(LONG NewState)
	{
		return _phost->TxDeactivate(NewState);
	}
	...
	BOOL		IsAutoKeyboard()
	{
		return _fAutoKeyboard;
	};
	//#endif

#ifdef DBCS
	BOOL	CALLBACK CmdDisplayIMEString(HWND hwnd, UINT msg, WPARAM wparam,
		LPARAM lparam, const LPCOMPOSITION lpComp, LPvoid);
	void	CmdReinit(BOOL fMode);
#endif

	// Selection access
	CTxtSelection *GetSel();
	CTxtSelection *GetSelNC()
	{
		return _psel;
	}
...
private:
	...
	HRESULT	OnTxSysKeyDown(WORD vkey, DWORD dwFlags);
	...
	LONG 	OnSetCharFormat(WPARAM wparam, LPARAM lparam, IUndoBuilder *publdr);
	LONG 	OnSetParaFormat(WPARAM wparam, LPARAM lparam, IUndoBuilder *publdr);
...
#if defined(PENWIN20) || defined(WIN16)
	void	OnCorrectText(LPRC prc);
#endif

#ifdef DBCS
	void	OnGetCompositionColor(COMPCOLOR  *lpCompColor);
	void	OnSetCompositionColor(COMPCOLOR  *lpCompColor);
	BOOL	OnString(LONG cchSz, LPCSTR sz);
	BOOL	OnStringWithCol(LONG cchSz, LPCSTR sz, COLORREF fbc, DWORD dwEffects);
#endif
...
public:
...
#ifdef DBCS
	VEDITWORDBREAKPROC 	_pfnWB;		// word break procedure
	...
	//LONG	_xLeftFmtRange;			// left edge of area from EM_FORMATRANGE
	//LONG	_yTopFmtRange;			// top edge of area from EM_FORMATRANGE
	//MRECT	_mrcPageFR;				// FORMATRANGE.rcPage


	//$ REVIEW: I needed to use these variables to display IME composition strings.
	//			Also I need to confirm with DavidFu whether there is problem or not.
#ifdef DBCS
	BOOL	_fRedirect;
	BOOL	_fModeDefer;
	LONG	_ichComp;				// start position of IME composition string
	LONG	_cchComp;				// character number of IME composition
	CHARFORMAT _cfSave;				//  string
	CHARFORMAT _cfComp;
	COLORREF _crConvert[ccrIME];	// IME composition color.
	DWORD	_dwConvert[ccrIME];		// IME composition effects.
	LONG	_cpCaretSave;
	LPvoid	_lpPunctObj;			// Object of PUNCT Class
	HCURSOR _hcurIBeam;
#endif
...
#define TXTBITS (TXTBIT_RICHTEXT	  | \
	...
	WORD		_fSingleCodePage	: 1;	// if TRUE, then only allow a single code page
	// for editing.
