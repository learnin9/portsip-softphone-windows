...
public:
	...
	HRESULT StartDrag( CTxtRange *prg, IUndoBuilder *publdr );
	BOOL fInDrag();
...
protected:
	...
	LONG 	ReadPlainText( CTxtRange *prg, EDITSTREAM *pes, BOOL fTestLimit,
								IUndoBuilder *publdr);
	LONG	WritePlainText( CTxtRange *prg, EDITSTREAM *pes, BOOL fTextize = FALSE );

	CTxtEdit *		_ped;
	CDropTarget *	_pdt;		// the current drop target
	IDataObject *	_pdo;		// data object that may be on the clipboard.
	BOOL			_fUseLimit;	// Whether to use limit text in calculation
