...
#ifdef DBCS
	ZeroMemory(&_cfSave, sizeof(CHARFORMAT));
	ZeroMemory(&_cfComp, sizeof(CHARFORMAT));
	ZeroMemory(_crConvert, sizeof(COLORREF) * ccrIME);
	ZeroMemory(_dwConvert, sizeof(DWORD) * ccrIME);
#endif
	ZeroMemory(&_mousePt, sizeof(POINT));

	// 本cpp下面的void *_cdecl operator new (size_t size)，重写了new，会把new出来的内存清0.
	// 但发现如果把void *_cdecl operator new (size_t size)注释掉，然后在这里手动调用ZeroMemory
	// 虽然可以把内存清0，但RichEdit显示不出来。
	// 但是，重载了operatoe new后，全局都使用这个重载了的operatoe new
	// 而重载了的operatoe new会调用ZeroMemory把内存清零，这会导致把虚函数表也清零。所以我们不用它。
	// 解决方案是用类的构造函数清0
	//int cb = sizeof(CTxtEdit);
	//ZeroMemory(this, cb);
...
CtrlC:
		...
		case 'V':						// Ctrl-V => paste
CtrlV:
			if (IsntProtectedOrReadOnly(WM_PASTE, 0, 0))
			{
	...
	LONG yMeasurePerInch)
{
	...
	if (fSetDCWorked)
	{
		...
		if (_pdpPrinter->SetTargetDC(pfr->hdcTarget))
		{
			...
			cpReturn = _pdpPrinter->FormatRange(pfr->chrg.cpMin, pfr->chrg.cpMax);
...
LONG ValidateTextRange(TEXTRANGE *pstrg)
{
	...
	if (IsBadReadPtr(pstrg, sizeof(TEXTRANGE))
		|| (((pstrg->chrg.cpMax < 1) || (pstrg->chrg.cpMin < 0)
				|| (pstrg->chrg.cpMax <= pstrg->chrg.cpMin))
			&& !((pstrg->chrg.cpMax == -1) && (pstrg->chrg.cpMin == 0))))
	{
	...
	return pstrg->chrg.cpMax - pstrg->chrg.cpMin;
...
LONG CTxtEdit::OnGetSelText(TCHAR *psz)
{
	...
	return GetTextRange(crSel.cpMin, crSel.cpMax - crSel.cpMin + 1, psz);
}
...
LRESULT CTxtEdit::OnGetSel(LONG *pcpMin, LONG *pcpMost) const
{
	...
	return (crSel.cpMax > 65535l)	? (LRESULT) - 1
		: MAKELRESULT((WORD) crSel.cpMin, (WORD) crSel.cpMax);
}
...
LRESULT CTxtEdit::OnFindText(UINT msg, DWORD flags, FINDTEXTEX *pftex)
{
	...
	if (TxFindText(flags, pftex->chrg.cpMin, pftex->chrg.cpMax, (TCHAR *)pftex->lpstrText, &lres) != S_OK)
	{
		if (msg == EM_FINDTEXTEX)
		{
			pftex->chrgText.cpMin = -1;
			pftex->chrgText.cpMax = -1;
		}
		return -1;
	}
	...
	if (msg == EM_FINDTEXTEX)					// we send a message back to the doc to change
	{
		...
		pftex->chrgText.cpMax = lres + wcslen(pftex->lpstrText);
	}
