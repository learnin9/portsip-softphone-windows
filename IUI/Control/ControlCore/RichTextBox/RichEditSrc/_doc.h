 ...
 * in arrays (see CReArray) and are pointed to by CRunPtr's of various kinds.
...
class CTxtRun
{
	//@todo (alexgo) These maybe should be protected, not public.
	//@access Public Methods and Data
public:
	CTxtRun()
	{
		_cch = 0;   //@cmember Constructor
	}
 ...
 *
 *	@base public | CReArray<lt>CTxtBlk<gt>
 */
class CTxtArray : public CReArray<CTxtBlk>
{
	friend class CTxtPtr;
...
typedef CReArray<CFormatRun> CFormatRuns;
...
class CTxtStory
{
	//BUGBUG (alexgo) get rid of these friends
	friend class CTxtPtr;
	friend class CRchTxtPtr;
...
public:
	...
	//@todo (alexgo): get rid of this
	//@cmember	Bogus method.
	...
	CFormatRuns *GetPFRuns()
	{
		return _pPFRuns;
	}
	//@cmember	Get the Character Formatting runs
	CFormatRuns *GetCFRuns()
	{
		return _pCFRuns;
	}
	...
	//@access	Private Data
private:
	//@todo (alexgo): maybe let this be a simple string
