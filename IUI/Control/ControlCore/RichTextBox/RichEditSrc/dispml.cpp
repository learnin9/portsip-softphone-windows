 ...
 *	@doc INTERNAL
 *
 *	@module	DISPML.C -- CDisplayML class |
 *
	...
	LONG xWidthMax)		//@parm Max line width (not used for screen)
{
	...
	if (SetTargetDC(hdc))
	{
		...
		_xWidthMax = (xWidthMax <= 0) ? 0 : max(DXtoLX(_xWidthSys), xWidthMax);
	...
	CLed *pled)					//@parm Records & returns impact of edit on lines (can be NULL)
{
	...
	while (lT-- > 0 && rpOld > 0 && !rpOld[-1]._cchEOP)
	{
		cliBackedUp++;
		rpOld--;
		cchSkip += rpOld->_cch;
	...
	// allowed, then just go directly to background recalc. If there is no height,
	// we just go a head and calculate some lines anyway. This prevents any wierd
	// background recalcs from occuring when it is unnecessary to go into background
	// recalc.
	if (fWait && ((_yWait > 0) && (yHeight > _yWait)
			&& ((LONG) me.GetCp() > _cpWait)))
	{
	...
	while (cchEdit > 0)
	{
		...
		if (cchSkip > 0 && cchSkip >= cchNonWhite &&
			((NULL == _pddTarget) || !_pddTarget->IsValid()))
		{
			...
			if ((rpOld->_bFlags & fliOffScreenOnce) != 0)
			{
		...
		*pliNew = me;
		...
		if (cchSkip > 0)
		{
			...
			{
				// There was no match in the line, so this line is the first
				// edited.
				cchSkip = 0;
			}
		}
	...
	while ((LONG) me.GetCp() < cchText)
	{
			...
			&& rpOld->_cch != 0)
		{
			...
			// that if the new array doesn't have any lines, we have too look
...
LONG CDisplayML::LineCount() const
{
	...
	if (!_ped->IsRich())
	{
		// if the last line ends in a CR, then up the line count
		if (cLine)
		{
			if (Elem(cLine - 1)->_cchEOP)
			{
				cLine++;
			}
		}
	...
	LONG ili) 		//@parm Line we're interested in
{
	...
	yPos = _yScroll + _dyFirstVisible;
	cli = ili - _iliFirstVisible;

	for (ili = _iliFirstVisible; cli > 0; cli--, ili++)
	{
		yPos += Elem(ili)->_yHeight;
	}
...
/*
 *	CDisplayML::CpFromPoint(pt, ptp, prp, fAllowEOL)
 *
 *	@mfunc
	...
	BOOL fAllowEOL)			//@parm Click at EOL returns cp after CRLF
{
	...
	if (pli != NULL)
	{
		...
		me.SetCp(cp);
		cch = pli->CchFromXpos(me, pt.x + _xScroll - rcView.left, &dx);
...
//		client rect)
{
	...
	if ((yRenderHeight > yLine)
		&& (yRenderHeight > yLine + Elem(ili)->_yHeight))
	{
	...
	// will work because characters in ajoining runs are only truncated
	// if ExTextOut is trying to clear the display area at the same time.
	...
	if (fLinesToRender)
	{
		...
		for (; ili < lCount; ili++)
		{
			if (!re.RenderLine(GetAt(ili), ili == (LONG)Count() - 1))
			{
				break;
			}
			...
			AssertSz(re.GetCurPoint().y == yLi, "CDisplayML::RenderView() - y out of sync. with line table");
#endif // DEBUG
		}
	}
 ...
 *		TRUE if success
 */
BOOL CDisplayML::RecalcView(BOOL fUpdateScrollBars)
{
	...
	SELDISPLAYACTION selAction)	//@parm Describes what we are doing to the selection
{
	...
	while (cp < cpMost && rc.top < rcView.bottom && rp.IsValid())
	{
		...
		rc.bottom = min(rc.top + rp->_yHeight, rcView.bottom);
	...
	LONG yPos)		//@parm Thumb position (yPos <lt> 0 for EM_SCROLL behavior)
{
	...
	case SB_LINEDOWN:
		...
		if ((_iliFirstVisible + cliVisible < (LONG)Count())
			&& (0 == _dyFirstVisible))
		{
			dy = Elem(_iliFirstVisible + cliVisible)->_yHeight;
		}
		else if (cliVisible > 1)
		{
			dy = Elem(_iliFirstVisible)->_yHeight + _dyFirstVisible;
		}
		else
		{
	...
	case SB_LINEUP:
		...
		{
			dy = Elem(_iliFirstVisible - 1)->_yHeight;
		}
		else if (yScroll > 0)
		{
			dy = min(yScroll, _yHeightSys);
		}
		if (dy > _yHeightView)
		{
			dy = _yHeightSys;
...
/*
 *	CDisplayML::FractionalScrollView ( LONG yDelta )
 *
 *	@mfunc
...
/*
 *	CDisplayML::CalcYLineScrollDelta ( LONG cli )
 *
 *	@mfunc
 ...
 */
LONG CDisplayML::CalcYLineScrollDelta(LONG cli, BOOL fFractionalFirst)
{
...
/*
 *	CDisplayML::ScrollView(xScroll, yScroll, fTracking)
 *
 *	@mfunc
	...
	INT nBar) const		//@parm Scroll bar to interrogate (SB_VERT or SB_HORZ)
{
	...
	if ((nBar == SB_VERT) && _fVScrollEnabled)
	{
		if (_ped->TxGetScrollBars() & WS_VSCROLL)
		{
			...
			// Because thumb messages only allow 16-bits, we limit our scroll
			// range to 16-bit
	...
	if ((_ped->TxGetScrollBars() & WS_HSCROLL) && _fHScrollEnabled)
	{
		// Scroll range is maxium width plus room for the caret.
		lRange = max(0, _xWidth + dxCaret);

		// Since thumb messages are limited to 16-bit, limit the range to
		// 16-bit.
		lRange = min(lRange, _UI16_MAX);
	}
...
BOOL CDisplayML::VerifyFirstVisible()
{
	...
	{
		Tracef(TRCSEVINFO, "signma CLine._cch = %ld, CDisplay.cpFirstVisible = %ld", cchSum, _cpMin);
