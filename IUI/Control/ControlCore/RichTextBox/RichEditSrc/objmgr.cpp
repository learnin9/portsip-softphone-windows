	...
	DWORD	cpMost)	//@parm End of range
{
	...
	if (iObj <= iLast)
	{
		...
		if ((*_objarray.Elem(iObj))->GetCp() <= cpMost)
		{
...
#else
	LPOLESTR	pszApp,	//@parm the app name
	LPOLESTR	pszDoc)	//@parm the doc name
#endif
{
...
#else
		_pszApp = new OLECHAR[OLEstrlen(pszApp) + 1];
#endif
		if (_pszApp)
		{
#ifndef MACPORTREMOVE
...
#else
		_pszDoc = new OLECHAR[OLEstrlen(pszDoc) + 1];
#endif
		if (_pszDoc)
		{
#ifndef MACPORTREMOVE
	...
	DWORD cp)		//@parm hinting cp
{
	...
	for (; i <= stop ; i++)
	{
		...
		{
			pobj = *_objarray.Elem(i);
			if (PtInRect(pobj->GetPosRect(), pt) &&
