 ...
 *
 *	Revisions: <nl>
 *		murrays (7/6/95) autodoc'd and added RTF support
 *
 *	FUTURE (AlexGo): <nl>
...
//Local Prototypes
DWORD __stdcall ReadHGlobal(DWORD dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb);
DWORD __stdcall WriteHGlobal(DWORD dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb);
	...
	CTxtRange *prg)				// @parm range to copy to clipboard
{
	...
	if (precall)
	{
		...
		prg->GetRange(chrg.cpMin, chrg.cpMax);
...
void CLightDTEngine::FlushClipboard()
{
	...
	// this cast is safe since we only hang onto our own objects
	CDataTransferObj *pdo = (CDataTransferObj *)_pdo;
	ENSAVECLIPBOARD ens;
	...
	DWORD			dwFlags)	// @parm DWORD packed flags
{
	...
	EDITSTREAM	es;				// EDITSTREAM to wrap around hglobal for RTF
	...
	LPTSTR		ptext = NULL;
	READHGLOBAL	rhg;			// Struct needed for es.pfnCallback for RTF
	...
	for (i = 0; i < CFETC; i++, pfetc++)
	{
		...
		if (_ped->IsRich() || (g_rgDOI[i] & DOI_CANPASTEPLAIN))
		{
				...
				i == iUnicodeFETC || i == iRtfNoObjs)
			{
				...
				// TEXT does not comes back w/ trailing NULL.  Reallocate to insure trailing null.
				if (i == iAnsiFETC)
				{
					hGlobal = GlobalReAlloc(hGlobal, GlobalSize(hGlobal) + 2, GMEM_MOVEABLE | GMEM_ZEROINIT);
				}
			...
			prg->AdjustEndEOP(NONEWCHARS);				// Don't delete trail
			//  EOP in some cases
			switch (i)									// If RTF, wrap
			{
			//  EDITSTREAM around
			case iRtfNoObjs:							//  hGlobal & delegate
			case iRtfFETC:								//  to LoadFromEs()
				...
				LoadFromEs(prg, SFF_SELECTION | SF_RTF,
					&es, TRUE, publdr);
				hresult			= es.dwError;
				break;
			...
			case iUnicodeFETC:							// Unicode plain text
				prg->CleanseAndReplaceRange(-1, ptext,
					TRUE, publdr);
	...
	IUndoBuilder *publdr)		// @parm undo builder to receive antievents
{
	...
	if (hr != NOERROR || pdo == NULL)
	{
		...
		hr = RangeToDataObject(prg, SF_TEXT | SF_RTF, &pdo);
		if (hr != NOERROR)
		{
			return hr;
		}
	...
	cch = prg->GetRange(cpMin, cpMost);		// NB: prg is the selection
	cp1 = prg->GetCp();						// Save active end and signed
	cch1 = prg->GetCch();					//  length for Undo antievent
	...
	IUndoBuilder *publdr)		// @parm undo builder to receive antievents
{
	...
	if (lStreamFormat & SF_RTF)					// RTF case must precede
	{
		...
		// FUTURE (alexgo): support loading RTF into a plain text control
		...
		// here we do something a bit unusual for performance reason.
		...
		if (publdr && prg->_rpPF.IsValid() && !tp.IsAfterEOP())
		{
			...
			if ((hr = GetParaFormatCache(&ppfc)) != NOERROR)
			{
				Assert(cch == 0);
				goto Exit;
			}
		...
		{
			// if there was some text added, create an anti-event for
			// it and add it in.

			AssertSz(_ped->GetCallMgr()->GetChangeEvent(), "Something "
				"changed, but nobody set the change flag");
	...
	EDITSTREAM *pes)			// @parm edit stream to save to
{
	...
	{
		cch = WritePlainText(prg, pes, lStreamFormat & SF_TEXTIZED);
	}
	else
	{
		Assert(FALSE);
	...
	LONG 		lStreamFormat)	// @parm stream format to use for loading
{
	...
	whg.cb			= cb;
#ifndef MACPORTREMOVE
	whg.hglobal		= GlobalAlloc(GMEM_MOVEABLE + GMEM_SHARE, cb);
#else
	whg.hglobal		= GlobalAlloc(GMEM_MOVEABLE | GMEM_SHARE, cb);
#endif
	if (!whg.hglobal)
	...
	whg.hglobal = GlobalReAlloc(whg.hglobal, whg.cch, GMEM_MOVEABLE);
	return whg.hglobal;
}
	...
	IUndoBuilder *publdr)		// @parm undo builder to receive antievents
{
	...
	if (ped->_pDocInfo)								// Update code page if
	{
		uCpg = ped->_pDocInfo->wCpg;    //  defined
	}
	...
	while (fContinue)								//  from previous read
	{
		...
		if (uCpg != 1200 && cbRead)					// Multibyte of some kind
		{
			...
			// becasue it will return bad char.
		...
		else if (!ped->Get10Mode())					// Unless RE 1.0 mode,
		{
			...
			if (wszBuf[cchConv - 1] == CR)			// There's at least one
			{
				// Set it up for next buf
				cCR++;								//  in case CR of CRLF
				if (wszBuf[cchConv - 2] == CR)		// Got 2nd CR; might be
				{
...
LONG CLightDTEngine::WritePlainText(
	...
	BOOL		fTextize)		// @parm Should objects in range be textized
{
	...
	if (_ped->_pDocInfo)								// Update code page if
	{
		uCpg = _ped->_pDocInfo->wCpg;    //  defined
	}
	...
	while ((LONG)tp.GetCp() < cpMost)
	{
		...
		{
			cbConverted = MbcsFromUnicode(szBuf, WRITESIZE, wszBuf, cch, uCpg,
