	...
	DWORD cpFormatMax)	//@parm max cp of formatting changes
{
	...
	if (cp <= _cp)
	{
		// if we've been deleted, then do nothing for now.
		// TODO (alexgo): when we plug in undo's, we could
		// probably assert here that we are in an undo state.
 ...
 *
 *	@rdesc	void
 需要初始化成员，否则会用0xCD（1100 1101）来默认初始化
 这样就把_fInPlaceActive初始化成1了。
 */
COleObject::COleObject(
...
void COleObject::MeasureObj(
	const CDevDesc *pdd,	//@parm	the device to measure for
	LONG &xWidth,			//@parm the width of the object
	LONG &yHeight,
	SHORT yDescent)		   	//@parm the height of the object
{
	xWidth = pdd->HimetricXtoDX(_sizel.cx);
	yHeight = pdd->HimetricYtoDY(_sizel.cy);
	...
	RECT  *prcRender)       //@parm pointer to render rectangle
{
	...
	{
		// 如果OLE是原地激活，不要做任何事，服务器正在为我们绘图。
		// 在设置fDraw属性之前，我们也不会做任何事情
		return;
	}
	...
	{
		// 内部会调用到IViewObject2::Draw，我们从IViewObject2派生的OLE对象就可以绘制了。
	...
	EDITSTREAM *pes)
{
	...
	else
	{
		hGlobal = med.hGlobal;
#ifdef MACPORT
		// Mac note: med.hGlobal is a Mac Handle - we have decided that Forms/Richedit
		//          internal functions and classes will pass and store WLM HANDLES so
		//          we need to wrap this Handle before using it.
...
BOOL COleObject::HandleResize(const POINT &pt)
{
	...
	while (GetAsyncKeyState(vkey) & 0x8000)
	{
		...
		if (fBottom)
		{
			_rcPos.bottom = ptCur.y;
		}
		// Keep a minimun width and height
		if (_rcPos.right - _rcPos.left < pdp->_xWidthSys)
		{
			if (fLeft)
			{
				_rcPos.left = _rcPos.right - pdp->_xWidthSys;
			}
			if (fRight)
			{
				_rcPos.right = _rcPos.left + pdp->_xWidthSys;
			}
		}
		if (_rcPos.bottom - _rcPos.top < pdp->_yHeightSys)
		{
			if (fTop)
			{
				_rcPos.top = _rcPos.bottom - pdp->_yHeightSys;
			}
			if (fBottom)
			{
				_rcPos.bottom = _rcPos.top + pdp->_yHeightSys;
			}
		}
		...
		// Eat input messages
		if (PeekMessageA(&msg, 0, WM_KEYFIRST, WM_KEYLAST, PM_REMOVE) ||
			PeekMessageA(&msg, 0, WM_MOUSEFIRST,
				WM_MOUSELAST, PM_REMOVE | PM_NOYIELD) ||
			PeekMessageA(&msg, 0, WM_NCMOUSEFIRST,
