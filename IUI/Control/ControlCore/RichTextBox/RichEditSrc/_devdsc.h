...
protected:
	...
	HDC		_hdcMeasure123;	// hdc for measuring (used for metafiles).
...
public:
	...
	LONG 	HimetricXtoDX(LONG xHimetric) const { return ::HimetricXtoDX(xHimetric, _xPerInch); }
	LONG 	HimetricYtoDY(LONG yHimetric) const { return ::HimetricYtoDY(yHimetric, _yPerInch); }
	LONG	DXtoHimetricX(LONG dx)  const { return ::DXtoHimetricX(dx, _xPerInch); }
	LONG	DYtoHimetricY(LONG dy) const { return ::DYtoHimetricY(dy, _yPerInch); }

#ifdef DEBUG
	LONG 	DXtoLX(LONG x) const;
	LONG 	DYtoLY(LONG y) const;
#else
	LONG 	DXtoLX(LONG x) const	{return ((x * LX_PER_INCH) * 2 + _xPerInch) / (2 * _xPerInch);}
	LONG 	DYtoLY(LONG y) const	{return ((y * LY_PER_INCH) * 2 + _yPerInch) / (2 * _yPerInch);}
#endif
