...
#define TwipsFromHimetric(_hm)	((((long) (_hm)) * (long) 72) / (long) 127)
#define HimetricFromTwips(_tw)	((((long) (_tw)) * (long) 127) / (long) 72)
...
// Defines for PF.wEffects (RichEdit 1.0 wReserved) Names are RTF

#define	PFE_INTBL		0x4000
#define	PFE_CELL		0x8000
#define	PFE_ROW			0xc000

#define PFM_DIR			0x10000
#define PFM_SLMULT		0x20000
#define PFM_KEEPN		0x80000
#define PFM_PAGEBB		0x100000
#define PFM_NOLINE		0x200000
#define PFM_NOWIDCTLPAR	0x400000
#define PFM_HYPHPAR		0x800000
#define PFM_SBYS		0x1000000
...
extern const char szRTFSig[];
extern const char szEndGroupCRLF[];

#define szaCRLF		&szEndGroupCRLF[1]
#define cchRTFSig   5
...
#if ( cachParamMax - 1 ) < 10
#error "cachParamMax MUST be >= 10"
#endif
...
typedef struct _textfont
{
	...
	SHORT		itfi;					// @field index into tagged font table
...
/*
 *	@struct CPGCHAR |
 *		Locally used variable that contain code-page and char-set info.
 */
...
INT GetCodePage(BYTE);
BOOL IsCharSetValid(BYTE bCharSet);

typedef CReArray<TEXTFONT> TEXTFONTS;
typedef CReArray<COLORREF> COLORREFS;
...
protected:
	...
	BOOL ReadFontSubInfo(void);
	BOOL FParseFontName(TCHAR *pchBuf,
		TCHAR chDelimiter,
		TCHAR **pszName,
		BYTE &bCharSet,
		BOOL &fSetCharSet,
