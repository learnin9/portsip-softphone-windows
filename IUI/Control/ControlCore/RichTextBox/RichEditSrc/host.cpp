...
LOCAL BOOL GetIconic(HWND hwnd)
{
 ...
 */
LRESULT CTxtWinHost::OnNCCreate(HWND hwnd, const CREATESTRUCT *pcs)
{
...
LRESULT CTxtWinHost::TxWindowProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_KEYDOWN:
		TRACE(_T("Host Key Down %d 0x%08x\r\n"), wparam/*ImmGetVirtualKey(hwnd)*/, lparam);
		if (lparam == 0x00390001)
		{
			int n = 0;
		}
		if ((WORD) wparam == VK_PROCESSKEY)
		{
			TRACE(_T("Host Key Down VK_PROCESSKEY\r\n"));
		...
		{
			TRACE(_T("Host Key Down not VK_PROCESSKEY\r\n"));
	...
	case WM_CHAR:
		// 只有输入英文才会进WM_CHAR，输入中文时是不会进入的。
		TRACE(_T("Host Key up\r\n"));

		// check if we are waiting for 2 consecutive WM_CHAR messages
		if (IsAccumulateDBCMode())
		{
			TRACE(_T("Host Key up IsAccumulateDBCMode\r\n"));
			if (!_chLeadByte)
			{
				...
				{
					TRACE(_T("Host Key up !IsDBCSLeadByte\r\n"));
		...
		TRACE(_T("Host Key up onchar\r\n"));
		lres = OnChar((WORD) wparam, (DWORD) lparam);
		if (lres)							// Didn't process code:
		{
			TRACE(_T("Host Key up goto2\r\n"));
			goto serv;    //  give it to text services
		}
		break;
	...
	case WM_PAINT:
	{
		...
		if (!fInAssert)
		{
			...
			if (_hpal != NULL)
			{
				hpalOld = SelectPalette(hdc, _hpal, FALSE);
				RealizePalette(hdc);
			}
...
serv:
		{
			...
			if (msg == WM_IME_COMPOSITION && lparam == 0x808)
			{
				int n = 0;
			}
...
void CTxtWinHost::RegisterDragDrop()
{
	...
	if (!_fRegisteredForDrop && _pserv->TxGetDropTarget(&pdt) == NOERROR)
	{
		...
		// 必须调用OleInitialize(NULL)进行初始化，不能用CoInitialize or CoInitializeEx
 ...
 */
HRESULT	CTxtWinHost::TxGetUndoManager(
	IUndoActionManager **ppUndoMgr) //@parm Forms3 undo manager
{
	TRACEBEGIN(TRCSUBSYSHOST, TRCSCOPEEXTERN, "CTxtWinHost::TxGetUndoManager");
...
BOOL CTxtWinHost::TxIsDoubleClickPending()
{
	...
	if (PeekMessageA(&msg, _hwnd, WM_LBUTTONDBLCLK, WM_LBUTTONDBLCLK,
