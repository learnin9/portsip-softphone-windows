 ...
 *		on all methods in ITextRange, ITextSelection, ITextFont, and ITextPara.
		...
		CTxtRange类实现RichEdit的文本范围，该范围是对文档进行更改的主要渠道。
		该范围继承自rich-text ptr，添加了带符号长度的插入点char-format索引和一个引用计数，
		以实例化为TOM ITextRange时使用。范围对象还包含一个标志，
		用于显示范围是选择项（具有关联的屏幕行为）还是仅是简单范围。
		这种区别用于简化某些代码。某些方法是虚拟的，以允许CTxtSelection对象促进UI功能和选择更新。
		有关范围和选择对象以及ITextRange，ITextSelection，ITextFont和ITextPara中的所有方法的大量讨论，
		请参见tom.doc。
 */
...
public:
	...
	void	SetIgnoreFormatUpdate(BOOL fUpdate)
	{
		_fDontUpdateFmt = fUpdate;
	}
	...
	void	CheckChange(LONG cpSave);	//@cmember Used after _cp change to set
	...
	void 	SetExtend(unsigned f)
	{
		_fExtend = f;
	}
	DWORD	GetAdjustedTextLength() const
	{
	...
	BOOL	IsZombie()
	{
		return !GetPed();   //@cmember Is range zombied?
	}
