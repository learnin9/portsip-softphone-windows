...
//
#if !defined(MACPORT) && (defined(DEBUG) || defined(_DEBUG) )	//macport change until dbug32 accessed
...
// 原名叫CTrace，因为与mfc的CTrace重名，故改名为CReTrace
class DllExport CReTrace
{
public:
	CReTrace(DWORD, DWORD, DWORD, LPSTR) {}
	~CReTrace() {}
...
DllImport extern PFNASSERTHOOK pfnAssert;
DllImport extern PFNTRACEHOOK pfnTrace;
DllImport extern DWORD dwDebugOptions;
DllImport void WINAPI Dbug32AssertSzFn(LPSTR, LPSTR, int);
DllImport void WINAPI TraceMsg5(DWORD, DWORD, DWORD, LPSTR, int);
...
// #define TRACEBEGIN(ss, sc, sz)  CReTrace trc(MAKEFLAGS((ss), TRCSEVNONE,\
...
#define TRACEBEGINPARAM(ss, sc, sz, param) \
	CReTrace trc(MAKEFLAGS((ss), TRCSEVNONE,\
