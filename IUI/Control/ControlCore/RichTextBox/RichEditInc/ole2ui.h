...
#if defined(NEVER) && !defined(__cplusplus) && !defined( __TURBOC__)
...
// ole2.h's don't match, so I need this here
#ifdef	WIN16
...
BOOL OleUIInitialize(void);
BOOL OleUIUninitialize(void);
...
BOOL OleUIAddVerbMenu(LPOLEOBJECT lpOleObj,
							 LPSTR lpszShortType,
							 HMENU hMenu,
							 UINT uPos,
							 UINT uIDVerbMin,
...
void    OleUIMetafilePictIconFree(HGLOBAL);
BOOL    OleUIMetafilePictIconDraw(HDC, LPRECT, HGLOBAL, BOOL);
UINT    OleUIMetafilePictExtractLabel(HGLOBAL, LPSTR, UINT, LPDWORD);
HICON   OleUIMetafilePictExtractIcon(HGLOBAL);
BOOL    OleUIMetafilePictExtractIconSource(HGLOBAL,LPSTR,UINT FAR *);

#if defined(DBCS) && defined(WIN16)
HGLOBAL OleUIMetafilePictFromIconAndLabel(HICON, LPSTR, LPSTR, UINT);
#endif	


// defined in \capone\mapin\ansiole.c
#ifdef	WIN32
HGLOBAL OleMetafilePictFromIconAndLabelA(HICON hIcon, LPSTR szLabelA,
									LPSTR szSourceFileA, UINT iIconIndex);
#endif	
...
//API prototype
UINT OleUIInsertObject(LPOLEUIINSERTOBJECT);
...
UINT OleUIPasteSpecial(LPOLEUIPASTESPECIAL);
...
//API Prototype
UINT OleUIEditLinks(LPOLEUIEDITLINKS);
...
#define ID_EL_SOURCE                   219 //$ MAIL
#define ID_EL_TYPE                     223 //$ MAIL
...
//API prototype
UINT OleUIChangeIcon(LPOLEUICHANGEICON);
...
//API prototype
UINT OleUIConvert(LPOLEUICONVERT);
...
#ifdef WIN32
HICON WINAPI ExtractIconA(HINSTANCE hInst, LPCSTR lpszFile, UINT nIconIndex);
