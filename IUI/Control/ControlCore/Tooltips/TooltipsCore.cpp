...
// Tooltips Timer ID
// 这里的Pop是隐藏的意思，而不是弹出来的意思
#define TTT_INITIAL        1
#define TTT_RESHOW         2
#define TTT_POP            3
#define TTT_AUTOPOP        4	// Timer到时间时，自动隐藏，默认GetDoubleClickTime() * 10
...
typedef struct
{
	...
	// 当"工具"绑定CToolTipCtrlProp时，记录子窗口是否被创建了。
	BOOL bCreatedChildren;
	...
	// 最大Tooltips宽度，如果为-1，则根据文本内容自动计算。
...
BOOL IUI::InitToolTipsClass(HINSTANCE hInstance)
{
	WNDCLASS wc;

	// See if we must register a window class
	if (!GetClassInfo(hInstance, IUI_TOOLTIPS, &wc))
	{
		wc.lpfnWndProc = (WNDPROC)ToolTipsWndProc;
		wc.lpszClassName = IUI_TOOLTIPS;
...
void NEAR PASCAL ToolTips_NewFont(PToolTipsMgr pTtm, HFONT hFont)
{
	...
	if (!hFont)
	{
		...
		if (hFont == NULL)
		{
			hFont = IUIGetGloabelVar()->g_hfontSystem;
			pTtm->fMyFont = FALSE;
		}
	}
...
void NEAR PASCAL PopBubble(PToolTipsMgr pTtm)
{
	...
	if (IsWindowVisible(pTtm->ci.hwnd) && pTtm->pCurTool)
	{
		...
		IUI_SendNotifyEx(pTtm->pCurTool->hwnd, (HWND)(-1), TTN_POP, &nmhdr, 1);
	}
	...
	// 如果Tooltips窗口是由CToolTipCtrlProp创建的，在这里可以销毁子控件，并初置窗口
	pTtm->bCreatedChildren = FALSE;

	HWLWND hRoot = (HWLWND)SendMessage(pTtm->ci.hwnd, WM_GETCHILD, 0, 0);
	while (hRoot != NULL)
	{
		// 预先得到下一个兄弟控件
		HWLWND hNext = GetWindow(hRoot, GW_HWNDNEXT);
	...
	// 恢复成非Layered窗口
	ToolTipCtrl *pToolTipCtrl = (ToolTipCtrl *)Window::FromHandlePermanent(pTtm->ci.hwnd);
	pToolTipCtrl->UseLayeredWindow(FALSE);

	// 窗口背景之类的属性不需要重置。 因为如果本Tooltip窗口显示传统文本时，
	// 绘制的分支与显示成绑定的CToolTipCtrlProp不同，不会用到CToolTipCtrlProp设置的属性。
}
...
PToolTipsMgr NEAR PASCAL ToolTipsMgrCreate(HWND hwnd, CREATESTRUCT FAR *lpCreateStruct)
{
	...
	if (NULL != pTtm)
	{
		...
		pTtm->clrTipBk = IUIGetGloabelVar()->g_clrInfoBk;
		pTtm->clrTipText = IUIGetGloabelVar()->g_clrInfoText;

		// Setup the default tooltip text buffer
		pTtm->lpTipText = (LPTSTR)LocalAlloc(LPTR, INITIALTIPSIZE * sizeof(TCHAR));
		...
		::SendMessage(pTtm->ci.hwnd, WM_SET_PROJECT, 0, (LPARAM)lpCreateStruct->lpCreateParams);
	}
	return pTtm;
}
...
void NEAR PASCAL TTSetTimer(PToolTipsMgr pTtm, int id)
{
	...
	if (SetTimer(pTtm->ci.hwnd, id, iDelayTime, NULL) && (id != TTT_POP))
	{
		pTtm->idTimer = id;
...
// 判断“工具”是否被移走了。也就是鼠标是否还是激活的“工具”内。
BOOL NEAR PASCAL ToolHasMoved(PToolTipsMgr pTtm)
{
	...
	if (bVisible)
	{
		...
		{
			if (hwnd == TTWindowFromPoint(pTtm, &pTtm->pt))
			{
				RECT rc;
...
void NEAR PASCAL TTUnsubclassHwnd(HWND hwnd, HWND hwndTT, BOOL fForce)
{
	...
	if (IsWindow(hwnd) &&
		IUI_GetWindowSubclass(hwnd, TTSubclassProc, (UINT_PTR)hwndTT, (PULONG_PTR) &dwRefs))
	{
		if (!fForce && (dwRefs > 1))
		{
			IUI_SetWindowSubclass(hwnd, TTSubclassProc, (UINT_PTR)hwndTT, dwRefs - 1);
		}
		else
		{
			IUI_RemoveWindowSubclass(hwnd, TTSubclassProc, (UINT_PTR)hwndTT);
		}
	}
}
	...
	LPARAM lParam, UINT_PTR uIdSubclass, ULONG_PTR dwRefData)
{
	...
	return IUI_DefSubclassProc(hwnd, message, wParam, lParam);
}
...
void NEAR PASCAL TTSubclassHwnd(PTOOLINFO pTool, HWND hwndTT)
{
	...
	if (IsWindow(hwnd = TTToolHwnd(pTool)))
	{
		...
		IUI_GetWindowSubclass(hwnd, TTSubclassProc, (UINT_PTR)hwndTT, &dwRefs);
		IUI_SetWindowSubclass(hwnd, TTSubclassProc, (UINT_PTR)hwndTT, dwRefs + 1);
	}
}
...
// bRichTool: 是否是“富工具”，富工具表示ToolTips窗口可展示丰富的自定义内容。
LRESULT NEAR PASCAL AddTool(PToolTipsMgr pTtm, BOOL bRichTool, LPTOOLINFO lpToolInfo)
{
	LRESULT lResult;
	...
	// 分配空间，并加入到TOOLINFO数组中
	PTOOLINFO ptoolsNew = (PTOOLINFO)CCLocalReAlloc(pTtm->tools,
	...
	PTOOLINFO pTool = &pTtm->tools[pTtm->iNumTools];
	pTtm->iNumTools++;
	hmemcpy(pTool, lpToolInfo, lpToolInfo->cbSize);
	pTool->lpReserved = (void *)bRichTool;
	...
	// 因为pTool->lpszText现在指向外面的用户传入的字符串，
	// 所以需要把传入的字符串，硬拷贝到pTool->lpszText中。
	pTool->lpszText = NULL;
	...
	if (lpToolInfo->hwnd == NULL || !IsWindow(lpToolInfo->hwnd))
	{
#ifdef UNICODE
		lResult = NFR_UNICODE;
#else
...
LPTSTR NEAR PASCAL GetToolText(PToolTipsMgr pTtm, PTOOLINFO pTool)
{
	...
	if (pTool->lpszText == LPSTR_TEXTCALLBACK)
	{
		...
		IUI_SendNotifyEx(pTool->hwnd, (HWND)(-1),
			0, (NMHDR FAR *)&ttt,
		...
		if (ttt.lpszText == ttt.szText
			&& ttt.szText[0] == TEXT('\0')
			&& (pTool->uFlags & TTF_UNICODE)
			&& pTtm->ci.iVersion < 5
			&& IsToolbarWindow(pTool->hwnd))
		{
			IUI_SendNotifyEx(pTool->hwnd, (HWND)(-1), 0, (NMHDR FAR *)&ttt, FALSE);
		}
		...
		{
			LPTSTR psz = (LPTSTR)LocalReAlloc(pTtm->lpTipText,
...
LoadFromResource:
	...
	else
	{
		...
		if (NULL != pTool->lpszText && *pTool->lpszText)
		{
...
LPTSTR NEAR PASCAL GetCurToolText(PToolTipsMgr pTtm)
{
	...
	// 在调用GetToolText时，会向外面发送WM_NOTIFY通知，这时可能导致pTtm->pCurTool为NULL
	if (pTtm->pCurTool == NULL)
	{
		psz = NULL;
	}
...
CToolTipCtrlProp *NEAR PASCAL GetToolWindowProp(PToolTipsMgr pTtm, PTOOLINFO pTool)
{
	if (pTool == NULL)
	{
		return NULL;
	...
	return (CToolTipCtrlProp *)pTool->lParam;
}

CToolTipCtrlProp *NEAR PASCAL GetCurToolWindowProp(PToolTipsMgr pTtm)
{
	CToolTipCtrlProp *pWindowProp = NULL;
	if (pTtm->pCurTool != NULL)
	{
		pWindowProp = GetToolWindowProp(pTtm, pTtm->pCurTool);
	}
...
void NEAR PASCAL TTGetTipPosition(
	PToolTipsMgr pTtm,
	LPRECT lprc,
	int cxText,
	int cyText,
	...
	int *pyStem)
{
	...
	int iBubbleWidth =  2 * XTEXTOFFSET * IUIGetGloabelVar()->g_cxBorder + cxText + cxMargin;
	int iBubbleHeight = 2 * YTEXTOFFSET * IUIGetGloabelVar()->g_cyBorder + cyText + cyMargin;
	UINT uSide = (UINT)(-1);
	RECT rcTool;
	MONITORINFO mi;
	HMONITOR    hMonitor;
	...
	if (bBalloon  || pTtm->cchTipTitle)
	{
		...
		if (bBalloon)
		{
			...
			else
			{
				...
				{
					// don't let the stem be longer than the bubble -- looks ugly
					pTtm->iStemHeight = iBubbleHeight / 3;
				}
			}
		}
	}
	...
	{
		// now set it
		_GetCursorLowerLeft((LPINT)&lprc->left, (LPINT)&lprc->top,
			&iCursorWidth, &iCursorHeight);
		if (bBalloon)
		{
...
BOOL TTCreateTitleBitmaps(PToolTipsMgr pTtm)
{
	...
	if (pTtm->himlTitleBitmaps)
	{
		...
		// TODO:hicon = (HICON)LoadImage(NULL, MAKEINTRESOURCE(IDI_TITLE_INFO), IMAGE_ICON,
		//		TITLEICON_WIDTH, TITLEICON_HEIGHT, LR_DEFAULTCOLOR);
		//ImageList_AddIcon(pTtm->himlTitleBitmaps, hicon);
		//DestroyIcon(hicon);
		//hicon = (HICON)LoadImage(NULL, MAKEINTRESOURCE(IDI_TITLE_WARNING), IMAGE_ICON,
		//		TITLEICON_WIDTH, TITLEICON_HEIGHT, LR_DEFAULTCOLOR);
		//ImageList_AddIcon(pTtm->himlTitleBitmaps, hicon);
		//DestroyIcon(hicon);
		//hicon = (HICON)LoadImage(NULL, MAKEINTRESOURCE(IDI_TITLE_ERROR), IMAGE_ICON,
		//		TITLEICON_WIDTH, TITLEICON_HEIGHT, LR_DEFAULTCOLOR);
		//ImageList_AddIcon(pTtm->himlTitleBitmaps, hicon);
		//DestroyIcon(hicon);
		return TRUE;
...
BOOL TTRenderTitledTip(PToolTipsMgr pTtm, HDC hdc, BOOL bCalcRect, RECT *prc, UINT uDrawFlags)
{
	...
	if (pTtm->uTitleBitmap != TTI_NONE)
	{
		...
		{
			ImageList_Draw(pTtm->himlTitleBitmaps, pTtm->uTitleBitmap - 1,
	...
	if (pTtm->lpTipTitle[0] != TEXT('\0'))
	{
		...
		_ASSERT(pTtm->iTitleHeight != 0 || uFlags & DT_CALCRECT);
...
void NEAR PASCAL TTGetTipSize(PToolTipsMgr pTtm, PTOOLINFO pTool, LPTSTR lpstr, LPINT pcxText, LPINT pcyText)
{
...

	{
		...
		dwCustom = (DWORD)IUI_SendNotifyEx(pTool->hwnd, (HWND)(-1),
				0, (NMHDR *) &nm,
				(pTool->uFlags & TTF_UNICODE) ? 1 : 0);
	...
	*pcxText += IUIGetGloabelVar()->g_cxEdge;
	*pcyText += IUIGetGloabelVar()->g_cyEdge;

	// 非Rich工具，但使用CToolTipCtrlProp来创建的ToolTips控件，应用文本边距。
	BOOL bRichTool = (BOOL)pTool->lpReserved;
	if (!bRichTool && pTool->lParam != 0)
	{
		CToolTipCtrlProp *pToolTipCtrlProp = (CToolTipCtrlProp *)pTool->lParam;
		RECT rcTextMargin = pToolTipCtrlProp->m_rcTextMargin;
		*pcxText += (rcTextMargin.left + rcTextMargin.right);
		*pcyText += (rcTextMargin.top + rcTextMargin.bottom);
	}
}
...
LRESULT TTAdjustRect(PToolTipsMgr pTtm, BOOL fLarger, LPRECT prc)
{
	...
	rc.left   -= XTEXTOFFSET * IUIGetGloabelVar()->g_cxBorder + pTtm->rcMargin.left;
	rc.right  += XTEXTOFFSET * IUIGetGloabelVar()->g_cxBorder + pTtm->rcMargin.right;
	rc.top    -= YTEXTOFFSET * IUIGetGloabelVar()->g_cyBorder + pTtm->rcMargin.top;
	rc.bottom += YTEXTOFFSET * IUIGetGloabelVar()->g_cyBorder + pTtm->rcMargin.bottom;
...
void NEAR PASCAL DoShowBubble(PToolTipsMgr pTtm)
{
	...
	LPTSTR lpstr = GetCurToolText(pTtm);
	CToolTipCtrlProp *pWindowProp = GetCurToolWindowProp(pTtm);
	...
	else
	{
		...
		// 优先使用IUI窗口属性定义的Tooltips，如果窗口属性为NULL，
		// 则使用标准风格的Tooltips.
		if (NULL == pWindowProp)
		{
	...
	do
	{
		// TOOLINFO::lpReserved表示“工具”是否是“富工具”
		// （不显示文本，界面编辑器设计成什么样，就展示成什么样）
		BOOL bRichTool = (BOOL)pTtm->pCurTool->lpReserved;
		// 计算Tooltips需要的宽度
		if (bRichTool)
		{
			if (pWindowProp != NULL)
			{
				cxText = RECTWIDTH((RECT)pWindowProp->m_rcControl);
				cyText = RECTHEIGHT((RECT)pWindowProp->m_rcControl);
			}
			else
			{
				_ASSERT(FALSE);
		...
		if (!IUI_SendNotifyEx(pTtm->pCurTool->hwnd, (HWND)(-1),
				TTN_SHOW, &si.hdr,
				(pTtm->pCurTool->uFlags & TTF_UNICODE) ? 1 : 0))
		{
			...
			if (GetWindow(pTtm->ci.hwnd, GW_OWNER) != NULL)
			{
				uFlags |= SWP_NOZORDER;
			}
			...
			// 到这里，Tooltips窗口仍然是不可见的
			_ASSERT(!IsWindowVisible(pTtm->ci.hwnd));
		}
	...
	{
		// 在创建ToolTips时，可以指定淡入淡出或滑出Tooltips窗口
		// 并且淡入淡出优先级比滑出更高
		...
		// 如果快速滑到目标区域，则直接弹出Tooltips，忽略淡入淡出或测出效果
		// 如果慢速滑入，则以动画形式弹出ToolTips.
		if (dwDelta < TIMEBETWEENANIMATE)
		{
			fAllowFade = fAllowAnimate = FALSE;
		}

		// 以非常cool的动画显示Tooltips窗口
		// 给外部一个机会去控制如何展示Tooltips。
		NMHDR hdr;
		hdr.code = TTN_COOL_TOOLTIP_BUBBLE;
		hdr.hwndFrom = pTtm->ci.hwnd;
		hdr.idFrom = GetDlgCtrlID(pTtm->ci.hwnd);
		NMCOOLBUBBLE nmcb;
		nmcb.hdr = hdr;
		nmcb.rc = rc;
		nmcb.fAllowFade = fAllowFade;
		nmcb.fAllowAnimate = fAllowAnimate;
		LRESULT lr = SendMessage(pTtm->pCurTool->hwnd, WM_NOTIFY, hdr.idFrom, (LPARAM)&nmcb);
		if (lr == 0)
		{
	...
	if (pToolSrc && lpTool && lpTool->cbSize <= sizeof(TOOLINFO))
	{
		if (lpTool->cbSize >= sizeof(TOOLINFO) - sizeof(LPARAM))
		{
				...
				&& !IS_INTRESOURCE(pToolSrc->lpszText))
			{
				if (lpTool->lpszText)
				{
					StringCchCopy(lpTool->lpszText, 1024*1024, pToolSrc->lpszText);
				}
			}
			else
			{
...
void NEAR PASCAL TTHandleTimer(PToolTipsMgr pTtm, UINT_PTR id)
{
	...
	case TTT_INITIAL:
		// 如果ToolHasMoved返回FALSE，则执行case TTT_RESHOW分支
		if (ToolHasMoved(pTtm))
		{
...
int DrawToolTipsText(
	...
	DWORD dwCustomDraw,
	BOOL &bRet,
	BOOL bUpdateLayeredWindow)
{
	HBRUSH hbr;

	if (NULL != lpszStr && 0 != *lpszStr)
	{
		BOOL bUseDrawText = (nm.uDrawFlags != uDefDrawFlags ||
		...
		else
		{
			...
			int x = XTEXTOFFSET * IUIGetGloabelVar()->g_cxBorder + prcMargin->left;
			int y = YTEXTOFFSET * IUIGetGloabelVar()->g_cyBorder + prcMargin->top;
			...
			if (!TTRenderTitledTip(pTtm, hdc, FALSE, &rc, uDefDrawFlags))
			{
				...
				{
					DrawFormatText(hdc, lpszStr, lstrlen(lpszStr), &rc, CIUIRect(0, 0, 0, 0),
						TAH_CENTER, TAV_CENTER, DT_SINGLELINE,
						pTtm->hFont, pTtm->clrTipText, TRUE);
				}
				else
				{
					DrawFormatText(hdc, lpszStr, lstrlen(lpszStr), &rc, CIUIRect(0, 0, 0, 0),
						TAH_CENTER, TAV_CENTER, DT_SINGLELINE,
						pTtm->hFont, pTtm->clrTipText, FALSE);
			...
			dwCustomDraw & CDRF_NOTIFYPOSTPAINT)
		{
			...
			IUI_SendNotifyEx(pTtm->pCurTool->hwnd, (HWND)(-1),
				0, (NMHDR *) &nm,
				(pTtm->pCurTool->uFlags & TTF_UNICODE) ? 1 : 0);
		}
...
// 绘制Tooltips窗口
BOOL TTRender(PToolTipsMgr pTtm, HDC hdc, LPCRECT lprcPaint)
{
	BOOL bRet = FALSE;
	RECT rc;

	if (NULL == pTtm->pCurTool)
	{
		return FALSE;
	}

	CToolTipCtrlProp *pWindowProp = GetCurToolWindowProp(pTtm);
	LPTSTR lpszStr = lpszStr = GetCurToolText(pTtm);
	if (NULL == pWindowProp)
	{
		if (NULL == lpszStr || 0 == *lpszStr)
		{
	...
	//
	// 通过向外部发送NM_CUSTOMDRAW通知，给外部一个机会，绘制Tooltips窗口
	//
	nm.nmcd.hdr.hwndFrom = pTtm->ci.hwnd;
	...
	// ADJUSTRECT!  Keep TTAdjustRect and TTGetTipPosition in sync.
	nm.nmcd.rc.left   = rc.left   + XTEXTOFFSET * IUIGetGloabelVar()->g_cxBorder + prcMargin->left;
	nm.nmcd.rc.right  = rc.right  - XTEXTOFFSET * IUIGetGloabelVar()->g_cxBorder - prcMargin->right;
	nm.nmcd.rc.top    = rc.top    + YTEXTOFFSET * IUIGetGloabelVar()->g_cyBorder + prcMargin->top;
	nm.nmcd.rc.bottom = rc.bottom - YTEXTOFFSET * IUIGetGloabelVar()->g_cyBorder - prcMargin->bottom;
	...
	dwCustomDraw = (DWORD)IUI_SendNotifyEx(pTtm->pCurTool->hwnd, (HWND)(-1),
			0, (NMHDR *) &nm,
			(pTtm->pCurTool->uFlags & TTF_UNICODE) ? 1 : 0);
	...
	if (NULL != pWindowProp)
	{
		// 因为一个Tooltips窗口，可能包含多个“工具”提示。而每个“工具”提示的内容又可能不同
		// 所以，在Tooltips窗口渲染时，才去创建子控件，在窗口隐藏时，销毁子控件
		Window *pWindow = Window::FromHandlePermanent(pTtm->ci.hwnd);

		if (!pTtm->bCreatedChildren)
		{
			// 绑定窗口风格，并创建子控件，
			pWindow->GetPaintManager()->InitWindow(pTtm->ci.hwnd, pWindowProp);
			pTtm->bCreatedChildren = TRUE;
		}

		if ((bool)pWindowProp->m_bLayeredWindow
			&& (bool)pWindowProp->m_bUpdateLayeredWindow)
		{
			::SetProp(pTtm->ci.hwnd, _T("UpdateLayeredWindow"), (HANDLE)1);

			pWindow->UseLayeredWindow(TRUE);

			HDC hdcTemp = ::GetDC(pTtm->ci.hwnd);
			HDC hdcMem = CreateCompatibleDC(hdcTemp);
			HBITMAP hbmpMem = CreateCompatibleBitmap(hdcTemp, RECTWIDTH(rc), RECTHEIGHT(rc));
			//void *pBits = NULL;
			...
			pWindow->SendMessage(WM_PRINT, (WPARAM)hdcMem, 0);

			if (!bRichTool)
			{
				// 计算坐标、绘制文本
				DrawToolTipsText(pTtm, hdcMem, lprcPaint, lpszStr, nm, uDefDrawFlags,
					rc, uFlags, prcMargin, dwCustomDraw, bRet, TRUE);
			}

			// Update Layered Window
			SIZE sizeWindow = {RECTWIDTH(rc), RECTHEIGHT(rc)};
			POINT ptSrc = {0, 0};
			...
			BOOL bRet = ::UpdateLayeredWindow(pTtm->ci.hwnd, hdcTemp,
			...
			::DeleteDC(hdcMem);
			hdcMem = NULL;
			::ReleaseDC(pTtm->ci.hwnd, hdcTemp);
			hdcTemp = NULL;
		}
		else
		{
			pWindow->UseLayeredWindow(FALSE);
			pWindow->SendMessage(WM_PRINT, (WPARAM)hdc, 0);

			if (!bRichTool)
			{
				// 计算坐标、绘制文本
				DrawToolTipsText(pTtm, hdc, lprcPaint, lpszStr, nm, uDefDrawFlags,
					rc, uFlags, prcMargin, dwCustomDraw, bRet, FALSE);
			}
...
void TTOnPaint(PToolTipsMgr pTtm)
{
	...
	if (!TTRender(pTtm, hdc, &ps.rcPaint))
	{
...
LRESULT WINAPI ToolTipsWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	PTOOLINFO pTool;
	PTOOLINFO pToolSrc;
	PToolTipsMgr pTtm = (PToolTipsMgr)GetWindowPtr(hwnd, 0);
	...
	case TTM_ADDTOOL:
		...
		return AddTool(pTtm, wParam, (LPTOOLINFO)lParam);
	...
	case TTM_GETCURRENTTOOLA:
		if (lParam)
		{
			return (LRESULT)(UINT)CopyToolInfoA(pTtm->pCurTool,
					(LPTOOLINFOA)lParam, pTtm->ci.uiCodePage);
		}
		else
		{
	...
	case TTM_ENUMTOOLSA:
	{
		if (wParam >= 0 && wParam < (UINT)pTtm->iNumTools)
		{
			...
			return (LRESULT)(UINT)CopyToolInfoA(pToolSrc,
					(LPTOOLINFOA)lParam, pTtm->ci.uiCodePage);
		}
		return FALSE;
	}
...
#define lphitinfoA ((LPHITTESTINFOA)lParam)
		...
		pTool = GetToolAtPoint(pTtm, lphitinfoA->hwnd,
				lphitinfoA->pt.x, lphitinfoA->pt.y, HTERROR, TRUE);
		if (pTool)
		{
...
#define lphitinfo ((LPHITTESTINFO)lParam)
		...
		pTool = GetToolAtPoint(pTtm, lphitinfo->hwnd,
				lphitinfo->pt.x, lphitinfo->pt.y, HTERROR, TRUE);
		if (pTool)
		{
	...
	case TTM_GETTEXT:
	{
		...
		if (lpszTemp)
		{
			StringCchCopy((((LPTOOLINFO)lParam)->lpszText), 1024 * 1024, lpszTemp);
		}
	}
	break;
...
#undef lpmsg
	...
	case TTM_GETBUBBLESIZE:
		if (lParam)
		{
			...
			if (pTool)
			{
				...
				iBubbleWidth =  2 * XTEXTOFFSET * IUIGetGloabelVar()->g_cxBorder + cxText + cxMargin;
				iBubbleHeight = 2 * YTEXTOFFSET * IUIGetGloabelVar()->g_cyBorder + cyText + cyMargin;
	...
	case WM_MOUSEMOVE:
		...
		if (!(pTtm->dwFlags & TRACKMODE)
			&& pTtm->pCurTool
			&& !(pTtm->pCurTool->uFlags & TTF_TRANSPARENT))
		{
			PopBubble(pTtm);
		}
	// fall through
	...
	case WM_NOTIFYFORMAT:
		...
		else if (lParam == NF_REQUERY)
		{
			...
			return IUI_CIHandleNotifyFormat(&pTtm->ci, lParam);
		}
		return 0;
	...
	{
		CIUIRect rcClient;
		GetClientRect(pTtm->ci.hwnd, rcClient);
		TTRender(pTtm, (HDC)wParam, rcClient);
	}
	break;
	...
	{
		LRESULT lres;
		if (IUI_CCWndProc(&pTtm->ci, uMsg, wParam, lParam, &lres))
		{
			return lres;
		}
	}
