...
void NEAR PASCAL isgoodbuddy(PUDSTATE np)
{
	...
	if (GetParent(np->hwndBuddy) != np->hwnd->ci.hwndParent)
	{
#if defined(DEBUG) && !defined(WIN32)
		DebugOutput(DBF_ERROR | DBF_USER,
...
{
	if (np->hwnd->ci.style & UDS_AUTOBUDDY)
	{
		np->hwndBuddy = GetWindow(np->hwnd->ci.hwnd, GW_HWNDPREV);
	}
}
...
void NEAR PASCAL unachor(PUDSTATE np)
{
	...
	if (np->hwndBuddy && (np->hwnd->ci.style & (UDS_ALIGNLEFT | UDS_ALIGNRIGHT)))
	{
		GetWindowRect(np->hwndBuddy, &rcBuddy);
		GetWindowRect(np->hwnd->ci.hwnd, &rcUD);
		UnionRect(&rc, &rcUD, &rcBuddy);
		ClientToScreen(np->hwnd->ci.hwndParent, &rc);
...
void NEAR PASCAL anchor(PUDSTATE np)
{
	...
	isgoodbuddy(np);
	nHasBorder = (np->hwnd->ci.style & WS_BORDER) == WS_BORDER;

	bAlignToBuddy = np->hwndBuddy && (np->hwnd->ci.style & (UDS_ALIGNLEFT | UDS_ALIGNRIGHT));

	if (bAlignToBuddy)
	{
		if ((np->uClass == CLASS_EDIT) ||
			(GetExStyle(np->hwndBuddy) & WS_EX_CLIENTEDGE))
		{
		...
		if ((np->uClass == CLASS_EDIT) || (GetStyle(np->hwndBuddy) & WS_BORDER))
		{
			// BUGBUG for full generalization, should handle border AND clientedge

			nOver = IUIGetGloabelVar()->g_cxBorder * (np->fSunkenBorder ? 2 : 1);
			np->fSharedBorder = TRUE;

			// turn off border styles...
			np->hwnd->ci.style &= ~WS_BORDER;

			SetStyle(np->hwnd->ci.hwnd, np->hwnd->ci.style);
			SetExStyle(np->hwnd->ci.hwnd, GetExStyle(np->hwnd->ci.hwnd) & ~(WS_EX_CLIENTEDGE));
		}
	}
	else
	{
		GetWindowRect(np->hwnd->ci.hwnd, &rc);
	}
	...
	//
	if (IS_WINDOW_RTL_MIRRORED(np->hwnd->ci.hwndParent))
	{
		rc.left = rc.right;
	}

	ScreenToClient(np->hwnd->ci.hwndParent, (LPPOINT)&rc.left);
	rc.right = rc.left + nWidth;

	if (bAlignToBuddy)
	{
		nWidth = IUIGetGloabelVar()->g_cxVScroll - IUIGetGloabelVar()->g_cxBorder;
		...
		if (np->hwnd->ci.style & UDS_ALIGNLEFT)
		{
			// size buddy to right
	...
	else if (!(np->hwnd->ci.style & UDS_HORZ))
	{
		nWidth = IUIGetGloabelVar()->g_cxVScroll + 2 * nHasBorder;
	}

	SetWindowPos(np->hwnd->ci.hwnd, NULL, rc.left, rc.top, nWidth, nHeight,
...
void NEAR PASCAL squish(PUDSTATE np, UINT bTop, UINT bBottom)
{
	...
	if (np->nUpper == np->nLower || !IsWindowEnabled(np->hwnd->ci.hwnd))
	{
		bTop = FALSE;
		bBottom = FALSE;
	}
	...
	if (np->fUp != bTop)
	{
		...
		MyNotifyWinEvent(EVENT_OBJECT_STATECHANGE, GetHostWnd(np->hwnd->ci.hwnd), OBJID_CLIENT, 1);
	}
	...
	if (np->fDown != bBottom)
	{
		...
		MyNotifyWinEvent(EVENT_OBJECT_STATECHANGE, GetHostWnd(np->hwnd->ci.hwnd), OBJID_CLIENT, 2);
	}

	if (bInvalidate)
	{
		np->dwStart = GetTickCount();
		InvalidateRect(np->hwnd->ci.hwnd, NULL);
	}
}
...
LRESULT NEAR PASCAL getint(PUDSTATE np, BOOL *pfError)
{
	...
	isgoodbuddy(np);
	if (np->hwndBuddy && np->hwnd->ci.style & UDS_SETBUDDYINT)
	{
		...
		*/
		...
		else
		{
			...
			case BASE_HEX:
				...
				{
					StringCchPrintf(p, MAX_INTLENGTH, TEXT("0x%08X"), pos);
				}
				else
				{
					StringCchPrintf(p, MAX_INTLENGTH, TEXT("0x%04X"), pos);
				}
				break;
			...
			default:
				...
				if (pos >= 1000 && !(np->hwnd->ci.style & UDS_NOTHOUSANDS))
				{
					...
					getthousands(szThousand);
					StringCchPrintf(szFmt, MAX_INTLENGTH, TEXT("%u"), pos);
					GetNumberFormat(LOCALE_USER_DEFAULT, 0, szFmt, &nf, p, MAX_INTLENGTH - 1);
				}
				else
				{
					StringCchPrintf(p, MAX_INTLENGTH, TEXT("%u"), pos);
				}
				break;
			}
	...
	*/
	...
	if (bChanged)
	{
		...
		*/
		...
		if (CCSendNotify(&np->hwnd->ci, UDN_DELTAPOS, &nm.hdr))
		{
			return;
		}
		...
		nudge(np);
		setint(np);
		if (np->hwnd->ci.style & UDS_HORZ)
		{
			FORWARD_WM_HSCROLL(np->hwnd->ci.hwndParent, np->hwnd->ci.hwnd, SB_THUMBPOSITION, np->nPos, SendMessage);
		}
		else
		{
			FORWARD_WM_VSCROLL(np->hwnd->ci.hwndParent, np->hwnd->ci.hwnd, SB_THUMBPOSITION, np->nPos, SendMessage);
		}

		MyNotifyWinEvent(EVENT_OBJECT_VALUECHANGE, GetHostWnd(np->hwnd->ci.hwnd), OBJID_CLIENT, 0);
	}
}
...
// Sets the new buddy
//
LRESULT NEAR PASCAL setbuddy(PUDSTATE np, HWLWND hwndBuddy)
{
	HWLWND hOldBuddy;
	TCHAR szClName[10];
	...
	if (hwndBuddy)
	{
		if (np->hwnd->ci.style & UDS_ARROWKEYS)
		{
			np->fSubclassed = TRUE;
		...
		if (!lstrcmpi(szClName, IUI_RICHTEXTBOX))
		{
			np->uClass = CLASS_EDIT;
		}
		else if (!lstrcmpi(szClName, IUI_LISTVIEW))
		{
...
void NEAR PASCAL PaintUpDownControl(PUDSTATE np, HDC hdc)
{
	...
	BOOL bEnabled = (np->nUpper != np->nLower) && IsWindowEnabled(np->hwnd->ci.hwnd);
	...
	GetClientRect(np->hwnd->ci.hwnd, &rcBtn);
	...
	if (np->fSharedBorder && np->fSunkenBorder)
	{
		UINT bf = BF_TOP | BF_BOTTOM | BF_ADJUST |
			(np->hwnd->ci.style & UDS_ALIGNLEFT ? BF_LEFT : 0) |
			(np->hwnd->ci.style & UDS_ALIGNRIGHT ? BF_RIGHT : 0);
		DrawEdge(hdc, &rcBtn, EDGE_SUNKEN, bf);
	}
	...
	if (np->hwnd->ci.style & UDS_HORZ)
	{
		...
		DrawFrameControl(hdc, &rc, DFC_SCROLL, uFlags);
	}
	else
	{
	...
	LPARAM lParam, UINT_PTR uIdSubclass, ULONG_PTR dwRefData)
{
	...
	case WM_KEYDOWN:
		...
		case VK_UP:
		case VK_DOWN:
			if (GetCapture(np->hwnd->ci.hwnd) != np->hwnd->ci.hwnd)
			{
				...
				*/
				...
				CCNotifyNavigationKeyUsage(&(np->hwnd->ci), UISF_HIDEFOCUS);
#endif
			}
			return (0L);
	...
	default:
		if (uMsg == IUIGetGloabelVar()->g_msgMSWheel && GetCapture(np->hwnd->ci.hwnd) != np->hwnd->ci.hwnd)
		{
...
UINT UD_HitTest(PUDSTATE np, int x, int y)
{
	...
	GetClientRect(np->hwnd->ci.hwnd, &rc);
	if (np->hwnd->ci.style & UDS_HORZ)
	{
...
void UD_Invalidate(PUDSTATE np, UINT uWhich, BOOL fErase)
{
	...
	InvalidateRect(np->hwnd->ci.hwnd, &rc);
}

void UD_OnMouseMove(PUDSTATE np, DWORD dwPos)
{
	if (np->hwnd->ci.style & UDS_HOTTRACK)
	{
...
/////////////////////////////////////////////////////////////////////////////
// SpinButton的窗口过程
LRESULT WINAPI IUI::SpinButton_WndProc(HWLWND hwnd, RoutedEventArgs *pe)
{
	PUDSTATE np = (PUDSTATE)GetMember(hwnd);
		...
		if (pe->eRoutingStrategy == RS_BUBBLE)
		{
			if (np == NULL)
			{
				if (uMsg == WM_CREATE)
				{
					...
					np->hwnd = hwnd;
					np->hwnd->ci.hwndParent = GetParent(hwnd);
...
#define lpCreate ((CREATESTRUCT FAR *)lParam)
	...
	if (np)
	{
		if ((uMsg >= WM_MOUSEFIRST) && (uMsg <= WM_MOUSELAST) &&
			(np->hwnd->ci.style & UDS_HOTTRACK) && !np->fTrackSet)
		{
			...
			tme.hwndTrack = GetHostWnd(np->hwnd->ci.hwnd);
	...
	case WM_LBUTTONDOWN:
	{
		...
		case CLASS_EDIT:
		case CLASS_LISTBOX:
			SetFocus(GetHostWnd(np->hwnd), np->hwndBuddy);
			break;
		}
	...
	case WM_TIMER:
	{
		...
		InflateRect(&rc,
			(IUIGetGloabelVar()->g_cxFrame + 1) / 2,
			(IUIGetGloabelVar()->g_cyFrame + 1) / 2);
		GetCursorPos(&pt);
		if (PtInRect(&rc, pt))
		{
			squish(np, !np->bDown, np->bDown);
...
EndScroll:
			...
			if (np->uClass == CLASS_EDIT)
			{
				SendMessage(np->hwndBuddy, EM_SETSEL, 0, -1);
			}

			if (np->hwnd->ci.style & UDS_HORZ)
				FORWARD_WM_HSCROLL(np->hwnd->ci.hwndParent, np->hwnd->ci.hwnd,
					SB_ENDSCROLL, np->nPos, SendMessage);
			else
				FORWARD_WM_VSCROLL(np->hwnd->ci.hwndParent, np->hwnd->ci.hwnd,
					SB_ENDSCROLL, np->nPos, SendMessage);
		}
		break;
	...
	case WM_UPDATEUISTATE:
		...
		CCOnUIState(&(np->hwnd->ci), WM_UPDATEUISTATE, wParam, lParam);
	...
	case UDM_SETBUDDY:
		return setbuddy(np, (HWLWND)wParam);
	...
	case UDM_SETACCEL:
		...
		if (wParam >= NUM_UDACCELS)
		{
			...
			else
			{
				...
				if ((np->hwnd->ci.style & UDS_ARROWKEYS) && np->hwndBuddy)
				{
					np->fSubclassed = TRUE;
					//SetWindowSubclass(np->hwndBuddy, ArrowKeyProc, 0, (ULONG_PTR)np);
				}
			}
		}
	...
	case WM_NOTIFYFORMAT:
		return CIHandleNotifyFormat(&np->hwnd->ci, lParam);
	...
	{
		LRESULT lres;
		if (CCWndProc(&np->hwnd->ci, uMsg, wParam, lParam, &lres))
		{
			return lres;
		}
	}
